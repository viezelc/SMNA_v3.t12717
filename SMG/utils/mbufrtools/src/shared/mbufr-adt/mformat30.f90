...
subroutine format_tab(sec4,nsubsets,cbt,ncbtmax,nrows,obs)
   !{ Interface
	...
	type(single),dimension(:,:), intent(out)::obs      !Tabela de valores.(1:nsubsets,1:ncols)
    !}	
    !{ variaveis locais
	integer :: i,j,bi,bj
	integer:: nobsmax
   ...
   do i=1,nsubsets		
      ...
      if (bi<=nobsmax) then 
        do bj=1,ncbtmax
          do j=1,sec4%nvars 
            if ((cbt(bj)%d==sec4%d(j,i)).and.(.not.(obs(bi,cbt(bj)%col)%l))) then          
                obs(bi,cbt(bj)%col)=fcbt(sec4%r(j,i),cbt(bj))
                if (obs(bi,cbt(bj)%col)%l) then 
                  exit 
                  exit 
                end if
             end if
...
!{ Variaveis da Interface
	...
	integer,                       intent(in)::ncbtmax  !Numero maximo de elementos em cbt desc_cols
	integer,                    intent(inout)::nrows    !Numero de linhas de obs
	type(single),dimension(:,:),intent(inout):: obs     !Tabela de valores (1:nsubsets,1:ndesc)
            ...
            do j=1,sec4qc%obs%nvars
              if (.not. atrib(j,i)) then 
                if ((sec4qc%qc(j,i)>minqc).or.(sec4qc%qc(j,i)==null)) then 
                  if ((cbt(bj)%d==sec4qc%obs%d(j,i)).and.(.not.obs(bi,cbt(bj)%col)%l)) then
...
function fcbt(value,cbt);type(single)::fcbt
!{ variaveis de interface
