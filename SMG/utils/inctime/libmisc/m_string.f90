...
MODULE m_string
  ...
  PUBLIC :: str2int      ! convert a number to string
  PUBLIC :: GetTokens    ! Get tokens by line
  PUBLIC :: find         ! find substring
  PUBLIC :: split        ! split string
  PUBLIC :: check        ! check if a substring is inside a string
...
CONTAINS
  ...
  SUBROUTINE str_template_(strg,nymd,nhms,fymd,fhms,jd,doy,label)
    ...
    IF(PRESENT(nymd)) THEN
       ...
       strg = replace_( strg, '%y2', int2str(year2,'(I2.2)') )
       strg = replace_( strg, '%y4', int2str(year4,'(I4.4)') )
       strg = replace_( strg, '%m1', int2str(month,'(I1)') )
       strg = replace_( strg, '%m2', int2str(month,'(I2.2)') )
       strg = replace_( strg, '%mc', mon_lc(month) )
       strg = replace_( strg, '%Mc', mon_wd(month) )
       strg = replace_( strg, '%MC', mon_uc(month) )
       strg = replace_( strg, '%d1', int2str(day,'(I1)') )
       strg = replace_( strg, '%d2', int2str(day,'(I2.2)') )

       !
       ! Initial/Analysis time template
       !

       strg = replace_( strg, '%iy2', int2str(year2,'(I2.2)') )
       strg = replace_( strg, '%iy4', int2str(year4,'(I4.4)') )
       strg = replace_( strg, '%im1', int2str(month,'(I1)') )
       strg = replace_( strg, '%im2', int2str(month,'(I2.2)') )
       strg = replace_( strg, '%imc', mon_lc(month) )
       strg = replace_( strg, '%iMc', mon_wd(month) )
       strg = replace_( strg, '%iMC', mon_uc(month) )
       strg = replace_( strg, '%id1', int2str(day,'(I1)') )
       strg = replace_( strg, '%id2', int2str(day,'(I2.2)') )
    ...
    IF(PRESENT(nhms)) THEN
       ...
       ! Default template variables
       !
       strg = replace_( strg, '%h1', int2str(hour,'(I1)') )
       strg = replace_( strg, '%h2', int2str(hour,'(I2.2)') )
       strg = replace_( strg, '%h3', int2str(hour,'(I3.3)') )
       strg = replace_( strg, '%n2', int2str(minute,'(I2.2)') )
       !
       ! Initial/Analysis time template
       !
       strg = replace_( strg, '%ih1', int2str(hour,'(I1)') )
       strg = replace_( strg, '%ih2', int2str(hour,'(I2.2)') )
       strg = replace_( strg, '%ih3', int2str(hour,'(I3.3)') )
       strg = replace_( strg, '%in2', int2str(minute,'(I2.2)') )
    ...
    IF ( PRESENT(fymd))THEN 
       ...
       strg = replace_( strg, '%fy2', int2str(fyear2,'(I2.2)') )
       strg = replace_( strg, '%fy4', int2str(fyear4,'(I4.4)') )
       strg = replace_( strg, '%fm1', int2str(fmonth,'(I1)') )
       strg = replace_( strg, '%fm2', int2str(fmonth,'(I2.2)') )
       strg = replace_( strg, '%fmc', mon_lc(fmonth) )
       strg = replace_( strg, '%fMc', mon_wd(fmonth) )
       strg = replace_( strg, '%fMC', mon_uc(fmonth) )
       strg = replace_( strg, '%fd1', int2str(fday,'(I1)') )
       strg = replace_( strg, '%fd2', int2str(fday,'(I2.2)') )
    ...
    IF (PRESENT(fhms) ) THEN
       ...
       strg = replace_( strg, '%fh1', int2str(fhour,'(I1)') )
       strg = replace_( strg, '%fh2', int2str(fhour,'(I2.2)') )
       strg = replace_( strg, '%fh3', int2str(fhour,'(I3.3)') )
       strg = replace_( strg, '%fn2', int2str(fminute,'(I2.2)') )

    END IF

    IF (PRESENT(label)) strg = replace_( strg, '%e', adjustl(trim(label)))

    IF (PRESENT(jd))THEN
       strg = replace_(strg, '%jdh', float2str(jd, '(F15.3)'))
       strg = replace_(strg, '%jd', int2str(int(jd), '(I10.3)'))
    ENDIF

    IF(PRESENT(doy))THEN
       strg = replace_(strg, '%jyh', float2str(doy, '(F15.3)'))
       strg = replace_(strg, '%jy', int2str(int(doy), '(I10.3)'))
    ENDIF
  ...
  ! !OUTPUT PARAMETERS:
    ...
    CHARACTER(len=*),INTENT(INOUT)  :: sfmt ! String

  !
  ! !REVISION HISTORY:
  !  Joao Gerd - 06Jan2023 - Codigo Inicial
  !    
  !EOP
  !---------------------------------------------------------------------!
  !BOC
  ...
  sfmt = mask
  sfmt = replace_( sfmt, '%y2', '(I2.2)') 
  sfmt = replace_( sfmt, '%y4', '(I4.4)') 
  sfmt = replace_( sfmt, '%m1', '(I1.1)') 
  sfmt = replace_( sfmt, '%m2', '(I2.2)')
  sfmt = replace_( sfmt, '%mc', '(A3)')
  sfmt = replace_( sfmt, '%Mc', '(A3)')
  sfmt = replace_( sfmt, '%MC', '(A3)')
  sfmt = replace_( sfmt, '%d1', '(I1.1)') 
  sfmt = replace_( sfmt, '%d2', '(I2.2)')
  sfmt = replace_( sfmt, '%h1', '(I1.1)') 
  sfmt = replace_( sfmt, '%h2', '(I2.2)')
  sfmt = replace_( sfmt, '%h3', '(I3.3)')
  sfmt = replace_( sfmt, '%n2', '(I2.2)')
  ...
  pure function replace_(strg,mask,repl) result(nstr)
    ...
    CHARACTER(len=*),INTENT(IN)  :: strg ! String
    CHARACTER(len=*),INTENT(IN)  :: mask ! maskout
    CHARACTER(len=*),INTENT(IN)  :: repl ! replacing string

    ! !OUTPUT PARAMETERS:

    CHARACTER(len=:),allocatable   :: nstr ! String
...
!-----------------------------------------------------------------------
     ...
     subString = lower_case(subStr)

     found = .false.
     do i = 1, strLen!-subStrLen
        istart = i
        iend   = min(i+subStrLen-1,strLen)
        if (trim(substring) == string(istart:iend))then
           found = .true.
           exit
        endif
     enddo
