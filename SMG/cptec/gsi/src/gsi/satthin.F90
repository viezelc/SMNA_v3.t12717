...
!EPV
! declare the dummy variables of routine read_gfssfc - workaround for r_single/r_kind issue
  real(r_kind), allocatable, dimension(:,:,:) :: dfact10_full,dsst_full,dsno_full
  real(r_kind), allocatable, dimension(:,:)   :: dveg_type_full
  real(r_kind), allocatable, dimension(:,:,:) :: dveg_frac_full
  real(r_kind), allocatable, dimension(:,:)   :: dsoil_type_full
  real(r_kind), allocatable, dimension(:,:,:) :: dsoil_temp_full,dsoil_moi_full
  real(r_kind), allocatable, dimension(:,:,:) :: dsfc_rough_full
  real(r_kind), allocatable, dimension(:,:)   :: dzs_full_gfs
!EPV
...
!EPV
            case('CPTEC')
                ...
                call read_bamsfc( mype_io, mype,  &
                                  dfact10_full,    & ! 10-meter wind factor
                                  dsst_full,       & ! surface temperature [k]
                                  dsno_full,       & ! snow depth [m]
                                  dveg_type_full,  & ! vegetation type  [1-13]
                                  dveg_frac_full,  & ! vegetation cover [-]
                                  dsoil_type_full, & ! soil type [1-9]
                                  dsoil_temp_full, & ! surface soil temperature [k]
                                  dsoil_moi_full,  & ! surface soil moisture [fraction]
                                  isli_full,      & ! land sea mask 
                                  dsfc_rough_full, & ! surface roughness length [cm]   
                                  dzs_full_gfs,    & ! Orography [ m ]
                                  use_sfc_any)


               fact10_full = real(dfact10_full,kind=r_single)
               sst_full = real(dsst_full,kind=r_single)
               sno_full = real(dsno_full,kind=r_single)
               veg_type_full = real(dveg_type_full,kind=r_single)
               veg_frac_full = real(dveg_frac_full,kind=r_single)
               soil_type_full = real(dsoil_type_full,kind=r_single)
               soil_temp_full = real(dsoil_temp_full,kind=r_single)
               soil_moi_full = real(dsoil_moi_full,kind=r_single)
               sfc_rough_full = real(dsfc_rough_full,kind=r_single)
               zs_full_gfs = real(dzs_full_gfs,kind=r_single)
...
!EPV
    allocate(dsfc_rough_full(nlat_sfc,nlon_sfc,nfldsfc))
    allocate(dzs_full_gfs(nlat_sfc,nlon_sfc))
    allocate(dsoil_moi_full(nlat_sfc,nlon_sfc,nfldsfc))
    allocate(dsoil_temp_full(nlat_sfc,nlon_sfc,nfldsfc))
    allocate(dveg_frac_full(nlat_sfc,nlon_sfc,nfldsfc))
    allocate(dsoil_type_full(nlat_sfc,nlon_sfc))
    allocate(dveg_type_full(nlat_sfc,nlon_sfc))
    allocate(dfact10_full(nlat_sfc,nlon_sfc,nfldsfc))
    allocate(dsno_full(nlat_sfc,nlon_sfc,nfldsfc))
    allocate(dsst_full(nlat_sfc,nlon_sfc,nfldsfc))
!EPV
...
!EPV
    if(allocated(dsst_full))       deallocate(dsst_full)
    if(allocated(dsno_full))       deallocate(dsno_full)
    if(allocated(dfact10_full))    deallocate(dfact10_full)
    if(allocated(dveg_type_full))  deallocate(dveg_type_full)
    if(allocated(dsoil_type_full)) deallocate(dsoil_type_full)
    if(allocated(dveg_frac_full))  deallocate(dveg_frac_full)
    if(allocated(dsoil_temp_full)) deallocate(dsoil_temp_full)
    if(allocated(dsoil_moi_full))  deallocate(dsoil_moi_full)
    if(allocated(dsfc_rough_full)) deallocate(dsfc_rough_full)
    if(allocated(dzs_full_gfs))    deallocate(dzs_full_gfs)
!EPV
