...
!                iuse_rad(j)=-1   !!!!!! line commented for assimilate amsua and atms, need evaluated others sensors
		 if(mype==mype_rad) then
                    write(6,*)'RADINFO WARNING: check inew_rad sensor/satelite/channel: ',nusis(j),nuchan(j)
                 endif
             end if
          end do
       end if
