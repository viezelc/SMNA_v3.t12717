...
!!
!> @brief Computes direct and inverse FFT transforms of sequences of 
...
module Mod_FastFourierTransform
  ...
  ! parameters
  integer, parameter :: p_r8 = selected_real_kind(15)              !< kind for 64bits
  integer, parameter :: p_nferr = 0                                !< Standard Error Print Out
  ...
  !> @brief createFFT
  !!
  !! @details Initializes internal values for FFTs of size nIn
  !!
  ...
  !> @brief factorize
  !!
  !! @details Factorizes nIn/2 in powers of 4, 3, 2, 5, if possible.
  ...
  !> @brief factorize
  !!
  !! @details  Next possible fft size and its factors
  !!
  !! @author Jose P. Bonatti \n
  !> Daniel M. Lamosa (last revision) \n
  !> Barbara A. G. P. Yamada (last revision)
  ...
  subroutine nextPossibleSize(nIn, nOut, powers)
    integer, intent(in ) :: nIn           !<
    integer, intent(out) :: nOut          !< positive even
  ...
  !> @brief trigFactors
  !!
  !! @details Sin and Cos required to compute FFT of size nIn
  !!
  !! @author Jose P. Bonatti \n
  !> Daniel M. Lamosa (last revision) \n
  !> Barbara A. G. P. Yamada (last revision)
  ...
  !> @brief destroyFFT
  !!
  !! @details Destrois internal data structure
  !!
  !! @author Jose P. Bonatti \n
  !> Daniel M. Lamosa (last revision) \n
  !> Barbara A. G. P. Yamada (last revision)
  ...
  subroutine joinFour(a, b, fout, ldout, n, nh, lot)
    ...
    integer :: i             !< Loop iterator
    integer :: j             !< Loop iterator
    real(kind=p_r8) :: scalR !< scale
    real(kind=p_r8) :: scalH !< half of scale
    real(kind=p_r8) :: c     !<
    real(kind=p_r8) :: s     !<
