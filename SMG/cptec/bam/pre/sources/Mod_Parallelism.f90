...
contains
  ...
  subroutine CreateParallelism(initializes_mpi)
    !# Creates Parallelism
    !# ---
    !# @info
    ...
    !# **Date**: mar/2019 </br>
    !# @endinfo
    logical, optional, intent(in) :: initializes_mpi
    integer :: ierror
    ...
    if (present(initializes_mpi) .and. initializes_mpi) then
      ! inicialize MPI
  ...
  subroutine parf_send_char(buff, buff_len, dest_host, tag)
    ...
    !# @endinfo
    integer(kind = p_i8), intent(in) :: buff_len
    character(len = buff_len), intent(in) :: buff
