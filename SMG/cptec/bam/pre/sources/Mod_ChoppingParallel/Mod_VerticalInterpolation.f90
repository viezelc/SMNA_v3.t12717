...
contains
  ...
  !CALL terp4 (ibdim,Im, Km1, Km2, 3+Nt,p1, z1, c1,p2, z2, c2,k1s)
  ...
  do k=1,Km2
    do i=1,Im
       ...
       if (dz > 0.0_p_r8) then
          ...
          u2(i,k)   = c1(i,k1,1)*EXP(-100.0_p_r8*dltdz*dz)
          v2(i,k)   = c1(i,k1,2)*EXP(-100.0_p_r8*dltdz*dz)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
       ...
       if(Km2 == Km1)then
           if(p2(i,k) <= p1(i,1) .and. p2(i,k) >= p1(i,Km1)  )then
              ...
              if( p2(i,k) < p1(i,Km1) .and. k > 1.and.k < Km2 )then
                      t2(i,k)  =0.25_p_r8*t2(i,k-2  )+0.5_p_r8*t2(i,k-1  )+0.25_p_r8*c2(i,k,3)!t1(i,k) 
                      q2(i,k,1)=0.25_p_r8*q2(i,k-2,1)+0.5_p_r8*q2(i,k-1,1)+0.25_p_r8*c2(i,k,4)!q1(i,k,1)
                      u2(i,k)  =0.25_p_r8*u2(i,k-2  )+0.5_p_r8*u2(i,k-1  )+0.25_p_r8*c2(i,k,1)!c2(i,k,1) !u1(i,k)
                      v2(i,k)  =0.25_p_r8*v2(i,k-2  )+0.5_p_r8*v2(i,k-1  )+0.25_p_r8*c2(i,k,2)!c2(i,k,2) !v1(i,k)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    enddo
  enddo
!PK    ! Copy Output Wind, Temperature, Specific Humidity and other Tracers
!PK    ! Except Below the Input Domain, Let Temperature Increase with a
!PK    ! Fixed Lapse Rate and Let the Relative Humidity Remain Constant

!PK   do k = 1, Km2
!PK     do i = 1, Im
!PK       k1=k1s(i,k)
!PK       t2(i,k)  =c2(i,k,3)
!PK       q2(i,k,1)=c2(i,k,4)
!PK       u2(i,k)  =c2(i,k,1)
!PK       v2(i,k)  =c2(i,k,2)
!PK       dz=z2(i,k)-z1(i,1)
!PK       if (dz >= 0.0_p_r8) then
!PK         t2(i,k)   = c2(i,k,3)
!PK         q2(i,k,1) = c2(i,k,4)
!PK         u2(i,k)  =c2(i,k,1)
!PK         v2(i,k)  =c2(i,k,2)
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!PK         if(Km2 == Km1)then
!PK          !IF(k > 1.and.k < Km2  .and. p2(i,k) <= p1(i,1) .and. p2(i,k) >= p1(i,Km1)  )THEN
!PK          if(p2(i,k) <= p1(i,1) .and. p2(i,k) >= p1(i,km1)  )then
!PK             if(k > 1.and.k < km2)then
!PK                t2(i,k)  =0.25_p_r8*c2(i,k-1,3)+0.5_p_r8*c2(i,k,3)+0.25_p_r8*c2(i,k+1,3)
!PK                q2(i,k,1)=0.25_p_r8*c2(i,k-1,4)+0.5_p_r8*c2(i,k,4)+0.25_p_r8*c2(i,k+1,4)
!PK                u2(i,k)  =0.25_p_r8*c2(i,k-1,1)+0.5_p_r8*c2(i,k,1)+0.25_p_r8*c2(i,k+1,1)!c2(i,k,1)
!PK                v2(i,k)  =0.25_p_r8*c2(i,k-1,2)+0.5_p_r8*c2(i,k,2)+0.25_p_r8*c2(i,k+1,2)!c2(i,k,2)
!PK             else if(k == 1 )then
!PK                t2(i,k)  =t1(i,1)
!PK                q2(i,k,1)=q1(i,1,1)
!PK                u2(i,k)  =u1(i,1)
!PK                v2(i,k)  =v1(i,1)
!PK             else if(k == km2 )then
!PK                t2(i,k)  =0.25_p_r8*t2(i,k-2  )+0.5_p_r8*t2(i,k-1  )+0.25_p_r8*c2(i,k,3)	    !t1(i,k) 
!PK                q2(i,k,1)=0.25_p_r8*q2(i,k-2,1)+0.5_p_r8*q2(i,k-1,1)+0.25_p_r8*c2(i,k,4)	    !q1(i,k,1)
!PK                u2(i,k)  =0.25_p_r8*u2(i,k-2  )+0.5_p_r8*u2(i,k-1  )+0.25_p_r8*c2(i,k,1)!c2(i,k,1) !u1(i,k)
!PK                v2(i,k)  =0.25_p_r8*v2(i,k-2  )+0.5_p_r8*v2(i,k-1  )+0.25_p_r8*c2(i,k,2)!c2(i,k,2) !v1(i,k)
!PK             end if
!PK         else !if(p2(i,k) > p1(i,1) .and. p2(i,k) <= p1(i,km1)  )then
!PK             if(k == 1 )then
!PK                t2(i,k)  =t1(i,1)
!PK                q2(i,k,1)=q1(i,1,1)
!PK                u2(i,k)  =u1(i,1)
!PK                v2(i,k)  =v1(i,1)
!PK             else if( p2(i,k) < p1(i,km1) )then
!PK                t2(i,k)  =0.25_p_r8*t2(i,k-2  )+0.5_p_r8*t2(i,k-1  )+0.25_p_r8*c2(i,k,3)	    !t1(i,k) 
!PK                q2(i,k,1)=0.25_p_r8*q2(i,k-2,1)+0.5_p_r8*q2(i,k-1,1)+0.25_p_r8*c2(i,k,4)	    !q1(i,k,1)
!PK                u2(i,k)  =0.25_p_r8*u2(i,k-2  )+0.5_p_r8*u2(i,k-1  )+0.25_p_r8*c2(i,k,1)!c2(i,k,1) !u1(i,k)
!PK                v2(i,k)  =0.25_p_r8*v2(i,k-2  )+0.5_p_r8*v2(i,k-1  )+0.25_p_r8*c2(i,k,2)!c2(i,k,2) !v1(i,k)
...
!PK             u2(i,k)  =u1(i,1)
!PK             v2(i,k)  =v1(i,1)
!PK          else if(k==km2 .and. km2 < km1)then
!PK             t2(i,k)  =c2(i,k,3)
!PK             q2(i,k,1)=c2(i,k,4)
!PK             u2(i,k)  =c2(i,k,1)
!PK             v2(i,k)  =c2(i,k,2)
!PK          else
!PK             t2(i,k)  =t1(i,1)*exp(dltdz*dz)
!PK             q2(i,k,1)=q1(i,1,1)*exp(dlpvdrt*(1.0_p_r8/t2(i,k)-1.0_p_r8/t1(i,1))-dz)
!PK             u2(i,k)  =u1(i,1)
!PK             v2(i,k)  =v1(i,1)
!PK          end if
!PK       end if
