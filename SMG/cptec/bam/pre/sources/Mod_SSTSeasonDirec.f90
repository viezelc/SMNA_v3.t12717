...
module Mod_SSTSeasonDirec
   ...
   public :: getNameSSTSeasonDirec, initSSTSeasonDirec, generateSSTSeasonDirec, shouldRunSSTSeasonDirec
   ...
   type SSTSeasonDirecNameListData
     integer            :: zMax                      ! Number of Layers of the Initial Condition for the Global Model
     integer            :: xDim                      ! Number of Longitudes For Climatological SST Data
     integer            :: yDim                      ! Number of Latitudes For Climatological SST Data
     integer            :: firstYear=1870            ! Initial Year of the temporal serie ( sst ) 1870 01 16 12
     integer            :: lastYear =2017            ! Last  Year of the temporal serie ( sst )    2017 12
     integer            :: firstYearClim=1870        ! Initial Year to sst climatology computation  1870 01 16 12
     integer            :: lastYearClim =2010        ! Last    Year to sst climatology computation     2017 12
     ...
     character(len = 7) :: preffix='GANLSMT'         ! Preffix of the Initial Condition for the Global Model
   ...
   end type SSTSeasonDirecNameListData

   type(varCommonNameListData)       :: varCommon
   type(SSTSeasonDirecNameListData)  :: var
   namelist /SSTSeasonDirecNameList/    var
   ...
   integer         :: yearInitialCondition           !ano da sst persistida 
   integer         :: monthInitialCondition          !mes da sst persistida
   integer         :: nMonthly             =1        ! ! Number of monthly before persistida
   

   integer         :: nMonth   =12                   !numero de meses da climatologia

   character (len = 12)  :: gradsTime='  Z         '
   character (len = 10)  :: trunc='T     L   '
   character (len = 7)   :: nLats='.G     '
   character (len = 10)  :: mskfmt = '(      I1)'
   character (len = 255) :: varName='SSTSeasonDirec'
   character (len = 16)  :: nameLSM='ModelLandSeaMask'
   character (len = 11)  :: fileClmSST='sstaoi.form'
   character (len = 10)  :: datePerc
   ...
   integer :: nfnml      ! To read namelist dirObs???namelist
...
!   test=.true.
   ...
   character(len = *), parameter :: headerMsg = 'SST Season Direc          | '
...
contains
  ...
  function getNameSSTSeasonDirec() result(returnModuleName)
    implicit none
    character (len = maxModuleNameLength) :: returnModuleName

    returnModuleName = "SSTSeasonDirec"
  end function getNameSSTSeasonDirec


  ! Eduardo Khamis - 16-05-2021
  function shouldRunSSTSeasonDirec() result(shouldRun)
    implicit none
    logical :: shouldRun

    shouldRun = .not. fileExists(getOutFileName())
  end function shouldRunSSTSeasonDirec


  ! Eduardo Khamis - 16-05-2021
  function getOutFileName() result(sstSeasonDirecOutFilename)
    implicit none
    character(len = maxPathLength) :: sstSeasonDirecOutFilename

    sstSeasonDirecOutFilename = trim(varCommon%dirModelIn) // trim(varName) // varCommon%date(1:8) // nLats
  end function getOutFileName
        

  ! Eduardo Khamis - 16-05-2021
  subroutine initSSTSeasonDirec (nameListFileUnit, varCommon_) 
    implicit none
    integer, intent(in) :: nameListFileUnit
    type(varCommonNameListData), intent(in) :: varCommon_
    
    read(unit = nameListFileUnit, nml = sstSeasonDirecNameList)
    ...
    datePerc=AddDate(varCommon%date,nMonthly)
    read(datePerc, fmt = '(I4.4,3I2.2)')yearInitialCondition,monthInitialCondition, day, hour
  
    if (mod(year,4) == 0) monthLength(2)=29
    if(yearInitialCondition<year)then
       write(*,*)'YearInitialConditionPERSI<Year'
       write(*,*)yearInitialCondition,'<',year
       stop
    end if
  ...
  end subroutine initSSTSeasonDirec
  ...
  subroutine printNameList()
    ...
    write (unit = nfprt, fmt = '(/,A)')    ' &SSTSeasonDirecNameList'
    ...
    write (unit = nfprt, fmt = '(A,I6)')   '     nMonthly  = ', nMonthly 
    write (unit = nfprt, fmt = '(A,I6)')   '     FirstYear = ', var%firstYear
    write (unit = nfprt, fmt = '(A,I6)')   '      LastYear = ', var%lastYear 
    write (unit = nfprt, fmt = '(A,I6)')   ' FirstYearClim = ', var%firstYearClim
    write (unit = nfprt, fmt = '(A,I6)')   '  LastYearClim = ', var%lastYearClim
  ...
  ! Eduardo Khamis - 16-05-2021
   character(len = 10)function AddDate(Cdate,nMonthly)
   implicit none
   character(len = *), intent(in   ) :: Cdate
   integer         , intent(in   ) :: nMonthly
   integer :: iyear,imon,iday,ihour
   read(Cdate(1:10),'(I4.4,3I2.2)')iyear,imon,iday,ihour
   imon=imon+nMonthly
   if(imon>12)then
      imon=(imon-12)
      iyear=iyear+1
   end if
   write(AddDate(1:10),'(I4.4,3I2.2)')iyear,imon,iday,ihour
  end function AddDate


  ! Eduardo Khamis - 16-05-2021
   function generateSSTSeasonDirec() result(isExecOk)
      implicit none
      logical :: isExecOk
      ...
      inFileName = trim(var%dirObsSSTnamelist)//'/sstssd.nml'
      nfnml = openFile(trim(inFileName), 'formatted', 'sequential', -1, 'read', 'old')
      if (nfnml < 0) return 
      read(nfnml,FNSSTNML)
      !open(11,FILE=trim(var%dirObsSSTnamelist)//'/sstssd.nml',STATUS='OLD')
      !read(11,FNSSTNML)
      ...
      outFileName = trim(varCommon%dirModelIn)//trim(varName)//varCommon%date(1:8)//'-'//datePerc(1:8)//nLats
   ...
   !
      ...
      if(var%firstYearClim >= nYearsFirstSerie .and. var%lastYearClim <= nYearsLastSerie)then
         write(*,*)'OK CLIMATOLOGY PERIOD ',var%firstYearClim,' >= ', nYearsFirstSerie,' .and. ',var%lastYearClim,' <=', nYearsLastSerie
      else
         write(*,*)'ERROR CLIMATOLOGY PERIOD ',var%firstYearClim,' >= ', nYearsFirstSerie,' .and. ',var%lastYearClim,' <=', nYearsLastSerie
         stop
      end if
   
   
      do iyear=var%firstYearClim,var%lastYearClim
         do imes=1,nMonth
            do j=1,var%yDim
               do i=1,var%xDim
      ...
      if(var%firstYear == nYearsFirstSerie .and. var%lastYear == nYearsLastSerie)then
         write(*,*)'OK PERSI SST PERIOD ',var%firstYear,' == ', nYearsFirstSerie,' .and. ',var%lastYear,' ==', nYearsLastSerie
   
      else
         write(*,*)'ERROR PERSI SST PERIOD ',var%firstYear,' == ', nYearsFirstSerie,' .and. ',var%lastYear,' ==', nYearsLastSerie
         stop
      end if
   
      do iyear=var%firstYear,var%lastYear+1
         firstMonthSerie=1 
         lastMonthSerie =12
         if(iyear == nYearsFirstSerie)then
            firstMonthSerie=nMonthFirstSerie
   ...
   end function generateSSTSeasonDirec
  ...
  ! Eduardo Khamis - 16-05-2021
   subroutine allocateData()
     implicit none
      ...
      allocate (sstpers (1:var%xDim,1:var%yDim,var%firstYear:var%lastYear+1,1:nMonth))
      allocate (sstanoma(1:var%xDim,1:var%yDim,var%firstYear:var%lastYear+1,1:nMonth))
      allocate (sststime(1:var%xDim,1:var%yDim,var%firstYear:var%lastYear+1,1:nMonth))
      allocate (sstclima(1:var%xDim,1:var%yDim,1:nMonth))
      allocate (sstcount(1:var%xDim,1:var%yDim,1:nMonth))
      sstpers=0.0_p_r4
      sstanoma=0.0_p_r4
      sststime=0.0_p_r4
      sstclima=0.0_p_r4
