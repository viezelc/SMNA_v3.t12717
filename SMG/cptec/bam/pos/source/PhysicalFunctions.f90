...
!-------------------------------------------------------------------------------
  ...
  FUNCTION CalcDewPoint(T0,Q0,P0,nCols)
    IMPLICIT NONE
    !  4 DEWPOINT AND FROSTPOINT FORMULAS
    !  Equations 2 and 3 are easily solved for vapor pressures at any given temperature, namely the
    !  dewpoint and frostpoint temperatures.  However, if vapor pressure is known with temperature
    ...
    ! es is the saturation vapor pressure in Pa
    INTEGER      , INTENT(in) :: nCols
    REAL(KIND=r8), INTENT(in) :: T0(nCols)!temperature  (K)
    REAL(KIND=r8), INTENT(in) :: Q0(nCols)!mixing ratio vapor (k/kg)
    REAL(KIND=r8), INTENT(in) :: P0(nCols)!pressure (mb)

    REAL(KIND=r8)             :: es (nCols)!mixing ratio vapor (g/kg)
    REAL(KIND=r8)             :: esu(nCols)!saturation vapor pressure in Pa
    INTEGER                   :: i
    REAL(KIND=r8)             :: CalcDewPoint(nCols)! Td is dewpoint temperature in Kelvin
    REAL(KIND=r8), PARAMETER  :: c0=  2.0798233e2_r8
    REAL(KIND=r8), PARAMETER  :: c1= -2.0156028e1_r8
    REAL(KIND=r8), PARAMETER  :: c2=  4.6778925e-1_r8
    REAL(KIND=r8), PARAMETER  :: c3= -9.2288067e-6_r8
    ...
    DO i=1, nCols
      esu(i)=fpvs2es5(T0(i))
      ! es(i)=fpvsq(T0(i))                  !saturation vapor pressure in Pascals
      !
      !
      es(i) = Q0(i) * ((100*P0(i))-esu(i))/0.622 !saturation vapor pressure in Pascals

    ENDDO
    DO i=1, nCols
      CalcDewPoint(i)= ((c0*(log(es(i)))**0) + (c1*(log(es(i)))**1) + (c2*(log(es(i)))**2) + (c3*(log(es(i)))**3))/ &
                       ((d0*(log(es(i)))**0) + (d1*(log(es(i)))**1) + (d2*(log(es(i)))**2) + (d3*(log(es(i)))**3))
    ENDDO
  END FUNCTION CalcDewPoint
