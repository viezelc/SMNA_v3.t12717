...
!      kl =  klast_four(ksg) - kfirst_four(ksg) + 1
  ...
  SUBROUTINE Rectrg (spec,grid,k,kg)

    INTEGER, INTENT(IN ) :: k, kg
    REAL(KIND=r8),    INTENT(IN ) :: spec(2*mymnmax,k)
    REAL(KIND=r8),    INTENT(OUT) :: grid(ibmax,kg,jbmax)
    IF (kg.eq.1) THEN
       CALL CreateSpecToGrid(0, 1, 0, 1)
       CALL DepositSpecToGrid(spec(:,1),grid(:,1,:))
     ELSE
       CALL CreateSpecToGrid(1, 0, 1, 0)
       CALL DepositSpecToGrid(spec,grid)
    ENDIF
    CALL DoSpecToGrid()
