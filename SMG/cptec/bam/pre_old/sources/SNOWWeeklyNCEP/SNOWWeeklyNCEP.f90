...
PROGRAM SNOWWeeklyNCEP
   ...
   CHARACTER (LEN=10) :: VarName='SNOWWeekly'

   CHARACTER (LEN=17) :: FileSNOW='gdas1.T  Z.snogrd'

   CHARACTER (LEN=12) :: TimeGrADS='  Z         '

   CHARACTER (LEN=18) :: NameNML='SNOWWeeklyNCEP.nml'
   ...
   REAL (KIND=r4), DIMENSION (:,:), ALLOCATABLE :: SNOW 

   INTEGER :: nferr=0    ! Standard Error Print Out
   INTEGER :: nfinp=5    ! Standard Read In
   INTEGER :: nfprt=6    ! Standard Print Out
   INTEGER :: nfsno=10   ! To Read Unformatted Weekly SNOW Data
   INTEGER :: nfout=20   ! To Write Unformatted Weekly SNOW Data
   ...
   ALLOCATE (SNOW(Idim,Jdim))

   FileSNOW(8:9)=Date(9:10)
   ...
   OPEN (UNIT=nfsno, FILE=TRIM(DirMain)//TRIM(DirPreIn)//FileSNOW//'.'//Date, &
         FORM='UNFORMATTED', ACCESS='SEQUENTIAL', &
         ACTION='READ', STATUS='OLD', IOSTAT=ios)
      IF (ios /= 0) THEN
      WRITE (UNIT=nferr, FMT='(3A,I4)') &
            ' ** (Error) ** Open file ', &
              TRIM(DirMain)//TRIM(DirPreIn)//FileSNOW//'.'//Date, &
            ' returned IOStat = ', ios
      STOP  ' ** (Error) **'
   END IF
   READ  (UNIT=nfsno)SNOW 
   CLOSE (UNIT=nfsno)
   ...
   IF (GrADS) THEN
      ...
      WRITE (UNIT=nfctl, FMT='(A)') '*'
      WRITE (UNIT=nfctl, FMT='(A)') 'TITLE NCEP Weekly SNOW'
      WRITE (UNIT=nfctl, FMT='(A)') '*'
      ...
      WRITE (UNIT=nfctl, FMT='(A)') 'SNOW 0 99 NCEP Weekly SNOW [kg/m2]'
      WRITE (UNIT=nfctl, FMT='(A)') 'ENDVARS'
      CLOSE (UNIT=nfctl)
   END IF

PRINT *, "*** SNOWWeeklyNCEP ENDS NORMALLY ***"

END PROGRAM SNOWWeeklyNCEP
