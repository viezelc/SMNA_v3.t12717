...
MODULE InputParameters
   ...
   REAL (KIND=r8), PUBLIC :: Undef, Lon0, Lat0, To, OCMSeaIce, OCMOpenWater, &
                             OCMSeaIceThreshold, LapseRate, EMRad1, EMRad12
   ...
   CHARACTER (LEN=8), PUBLIC :: VarName='OCMClima'
   ...
   CHARACTER (LEN=12), PUBLIC :: DirClmOCM='pre/databcs/' ! Climatological OCM Datain Directory

   CHARACTER (LEN=11), PUBLIC :: FileClmOCM='ersst.form'

   CHARACTER (LEN=20), PUBLIC :: FileClmOCMData='ocm_data.bin'

   CHARACTER (LEN=12) :: NameNML='OCMClima.nml'
   ...
   INTEGER,PUBLIC, PARAMETER :: nMon=12
   INTEGER,PUBLIC, PARAMETER :: nlevl=19
   ...
   INTEGER, PUBLIC :: nfclm=30   ! To Read Formatted Climatological OCM
   INTEGER, PUBLIC :: nfocm=31   ! To Read Formatted Climatological OCM
   INTEGER, PUBLIC :: nfsto=40   ! To Write Unformatted Gaussian Grid OCM
   INTEGER, PUBLIC :: nfout=50   ! To Write GrADS Topography, Land Sea, Se Ice and Gauss OCM
...
SUBROUTINE InitInputParameters ()
   ...
   Idim=360           ! Number of Longitudes For Climatological OCM Data
   Jdim=180           ! Number of Latitudes For Climatological OCM Data
   OCMSeaIce=-1.749_r8   ! OCM Value in Celsius Degree Over Sea Ice (-1.749 NCEP, -1.799 CAC)
   ...
   WRITE (UNIT=nfprt, FMT='(A,F6.3)') '  OCMSeaIce = ', OCMSeaIce
   ...
   To=273.15_r8
   OCMOpenWater=-1.7_r8+To
   OCMSeaIceThreshold=271.2_r8
