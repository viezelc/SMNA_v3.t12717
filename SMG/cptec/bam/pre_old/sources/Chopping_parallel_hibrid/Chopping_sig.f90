...
PROGRAM Chopping
  ...
  USE InputParameters, ONLY: InitParameters, i4, r4, r8, nferr, nfinp, nfprt, nftop, &
                             ...
                             DataCPT, DataInp, DataOut, DataOup, DataTop, DataSig, DataSigInp, &
...
!         ACCESS='SEQUENTIAL', ACTION='READ', STATUS='OLD', IOSTAT=ios)
    OPEN  (UNIT=nfsig, FILE=TRIM(DirSig)//TRIM(DataSig), &
           IOSTAT=ios)
    IF (ios /= 0) THEN
      WRITE (UNIT=nferr, FMT='(3A,I4)') ' ** (Error) ** Open file ', &
      ...
      STOP ' ** (Error) **'
    END IF
        if(myid.eq.0) WRITE(unit=nfprt,*) ' SigInterOut(k), SigLayerOut(k)'
       DO k=1,KmaxOut+1
          READ(UNIT=nfsig,*) SigInterOut(k), SigLayerOut(k)
          SigInterOut(k) = SigInterOut(k) / 100._r4  ! transform from Pa to mbar
          if(myid.eq.0) WRITE(unit=nfprt,*) SigInterOut(k), SigLayerOut(k)
       ENDDO
...
! END DO
 ...
 ! gTvirInp=gTvirInp/(1.0_r8+cTv*gSpHuInp(:,:,:,1))
  ...
  write(*,*) myid, ' KmaxInp ',KmaxInp, 'KmaxOut ',KmaxOut
  DO j=1,Jbmax
...
!   DO j=1,Jbmax
!      DO I=1,Ibmaxperjb(j)
!       m1 = max(m1,gtvirout(i,1,j))
!       m2 = min(m2,gtvirout(i,1,j))
!       m3 = max(m3,gtvirout(i,kmaxout,j))
!       m4 = min(m4,gtvirout(i,kmaxout,j))
!     END DO
!   END DO
!    write(nfprt,*) myid , 'max tvirout 1',m1, ' min ', m2
!    write(nfprt,*) myid , 'max tvirout kmaxout',m3, ' min ', m4
    CALL dumpgra
...
SUBROUTINE GDAStoGANL
  ...
  WRITE (UNIT=nfprt, *) 'IDSLInp ', IDSLInp   
  WRITE (UNIT=nfprt, *) 'IDVCINP ', IDVCINP   
  IF(myid.eq.0.and.IDVCInp == 2_i4)THEN
    WRITE (UNIT=nfprt, FMT='(/,A)')   'a_hybr  (in Pa):'
    WRITE (UNIT=nfprt, *) SigIInp * 100.0_r4
    WRITE (UNIT=nfprt, FMT='(/,A)')   'b_hybr  '
    WRITE (UNIT=nfprt, *) SigLInp
   ELSE
    WRITE (UNIT=nfprt, FMT='(/,A)')   'SigIInp:'
    WRITE (UNIT=nfprt, *) SigIInp
...
! SigIOut=SigInterOut*100._r4  ! convert back from mb to Pa
  SigIOut(1:kmaxout)=(SigLayerOut(1:kmaxout)+SigLayerOut(1:kmaxout+1))*0.5_r4  ! convert back from mb to Pa
  SigLOut=SigLayerOut

  IF (myid.eq.0) THEN
    WRITE (UNIT=nficw) ForecastDay, TimeOfDay, DateInitial, &
                       DateCurrent, SigLOut, SigIOut(1:kmaxout)
    ...
    WRITE (UNIT=nfprt, FMT='(/,A)')  ' A_hybr_Out (in Pa):'
    WRITE (UNIT=nfprt, FMT='(7F10.6)') SigInterOut*100._r4
    WRITE (UNIT=nfprt, FMT='(/,A)')  ' B_hybr_Out:'
...
SUBROUTINE ICRecomposition
  ...
  IMPLICIT NONE
    ...
    ENDDO
    CALL MsgOne('ICRdecomp', ' Fields deposited ')
    !$OMP END SINGLE
    CALL DoSpecToGrid()
    !$OMP BARRIER
    !$OMP SINGLE
...
!       m1 = max(m1,gPsfcInp(i,j))
!       m2 = min(m2,gPsfcInp(i,j))
       END DO
    END DO
!    write(nfprt,*) myid , 'max gps  ',m1, ' min ',m2
...
SUBROUTINE GetGrADSInp
  ...
  IF (myid.eq.0) THEN
    ...
    INQUIRE (IOLENGTH=IOL) gWorkprOut
    OPEN (UNIT=nfgrd, FILE=TRIM(DirGrd)//TRIM(DataOup)//'.GrADS', FORM='UNFORMATTED', &
        ACCESS='DIRECT', RECL=IOL, ACTION='WRITE', STATUS='REPLACE', IOSTAT=ios)
    IF (ios /= 0) THEN
      WRITE (UNIT=nferr, FMT='(3A,I4)') ' ** (Error) ** Open file ', &
                                      TRIM(TRIM(DirGrd)//TRIM(DataOup))//'.GrADS', &
                                      ' returned IOStat = ', ios
      STOP ' ** (Error) **'
    END IF
  ...
  IF (myid.eq.0) THEN
    OPEN (UNIT=nfctl, FILE=TRIM(DirGrd)//TRIM(DataOup)//'.GrADS.ctl', FORM='FORMATTED', &
        ACCESS='SEQUENTIAL', ACTION='WRITE', STATUS='REPLACE', IOSTAT=ios)
    IF (ios /= 0) THEN
      WRITE (UNIT=nferr, FMT='(3A,I4)') ' ** (Error) ** Open file ', &
                                      TRIM(TRIM(DirGrd)//TRIM(DataOup))//'.GrADS.ctl', &
                                      ' returned IOStat = ', ios
      STOP ' ** (Error) **'
    END IF
    WRITE (UNIT=nfctl, FMT='(A)') 'dset ^'//TRIM(DataOup)//'.GrADS'
    ...
    WRITE (UNIT=nfctl, FMT='(6F10.3)') SigInterInp+1000.0_r8*SigLayerInp
...
SUBROUTINE GetGrADSOut
  ...
  IF (myid.eq.0) THEN
    ...
    WRITE (UNIT=nfctl, FMT='(6F10.3)') SigInterOut+1000.0_r8*SigLayerOut
    WRITE (UNIT=nfctl, FMT='(3A)') 'tdef 1 linear ', Tdef,' 1dy'
    IF (NTracers == 1) THEN
      WRITE (UNIT=nfctl, FMT='(A)') 'vars 6'
    ELSE
      WRITE (UNIT=nfctl, FMT='(A,I5)') 'vars ',4+NTracers
    END IF
    WRITE (UNIT=nfctl, FMT='(A)') 'topo   0 99 Topography        '//TruncInp//' (m)'
    WRITE (UNIT=nfctl, FMT='(A)') 'pslc   0 99 Surface Pressure  '//TruncInp//' (hPa)'
    WRITE (UNIT=nfctl, FMT='(A,I3,A)') 'tvir ',KmaxOut,' 99 Virt Temperature  '// &
                                     TruncInp//' (K)'
    WRITE (UNIT=nfctl, FMT='(A,I3,A)') 'pres ',KmaxOut,' 99 Specific Humidity '// &
                                     TruncInp//' (kg/kg)'
