...
PROGRAM Chopping
  ...
  USE Utils, ONLY: NewSigma, SigmaInp, NewPs, coslat, rcl, lati,IJtoIBJB
  ...
  USE Sizes, ONLY: jbmax, Ibmaxperjb, mymmax, msinproc, mnmap, mymnmap, &
       ...
       iperijb,jperijb, myfirstlon,mylastlon,&
       imax, jmax,Ibmaxperjb,myfirstlat,myfirstlat_diag,mylastlat_diag,mylastlat

  IMPLICIT NONE

  INTEGER :: ios, nRec, IOL

  INTEGER :: i, j, k, nt, ierror, ib, jb,lrec
  ...
  IF (GetNewSig) THEN
     ...
     IF (ios /= 0) THEN
        ...
        STOP ' ** (Error) **'
     END IF
     IF(myid.EQ.0) WRITE(unit=nfprt,FMT='(A)') ' SigInterOut(k), SigLayerOut(k)'
     DO k=1,KmaxOut+1
        READ(UNIT=nfsig,FMT=*) SigInterOut(k), SigLayerOut(k)
        SigInterOut(k) = SigInterOut(k) / 100._r4  ! transform from Pa to mbar
        IF(myid.EQ.0) WRITE(unit=nfprt,FMT=*) SigInterOut(k), SigLayerOut(k)
     ENDDO
     !   READ  (UNIT=nfsig, FMT='(5F9.6)') DelSigmaOut
     CLOSE (UNIT=nfsig)
     IF (KmaxOut == KmaxInp) THEN
        !IF (MAXVAL(ABS(DelSigmaOut(1:KmaxOut)-DelSigmaInp(1:KmaxInp))) < 1.0E-04_r8) THEN
           IF (myid.EQ.0) THEN
...
CONTAINS
  ...
  SUBROUTINE ICRead_and_Chop(myid,ibMax,jbMax,GetNewTop)
    ...
    IF (GetNewTop.AND.havesurf) THEN
       WRITE (UNIT=nfprt, FMT='(/,A)')' Getting New Spec Topography'
    ...
    ELSE
       ...
       END IF
    END IF
    IF (SmoothTopo.AND.havesurf.AND..not.GetNewTop) THEN
       CALL SmoothCoef()
    END IF
