...
! DirSig=TRIM(DirHome)//'sources/Chopping/ '
  ...
  END IF
  DataTop='Topography.'//TruncOut(1:6)         ! Input Topography Spec File Name
  DataTopG='Topography.'//TRIM(TrGrdOutGaus)   ! Input Topography Grid File Name
  DataSigInp='DeltaSigma.'//TruncInp(7:10)     ! Delta Sigma File Input
  DataSig='HybridLevels.'//TruncOut(7:10)      ! Delta Sigma File Output
  ...
  IF (myid.eq.0) THEN
    WRITE (UNIT=nfprt, FMT='(A)')      ' '
    WRITE (UNIT=nfprt, FMT='(A,I5)')   '  ImaxInp    = ', ImaxOut
    WRITE (UNIT=nfprt, FMT='(A,I5)')   '  JmaxInp    = ', JmaxOut
