...
! use nemsio_module_mpi
 ...
 use nemsio_gfs
  ...
  IF (KmaxOut == KmaxInp .AND.(.NOT.GetNewTop .AND. .NOT.SmoothTopo)) VerticalInterp = .FALSE.
  ...
  ! Calculate pressure for each layer according to vertical coordinate
   ...
   ELSE IF ( IDVCInp == 2_i4 ) THEN
       ...
       DO j=1,Jbmax
          DO k=1,KmaxOutp
             DO I=1,Ibmaxperjb(j)
                gPresaux(i,k,j)=SigInterOut(k)+gPsfcOut(i,j)*SigLayerOut(k)
             END DO
          END DO
          IF ( IDSLInp == 2_i4 ) THEN
             ! Mean over two interfaces
             DO k=1,KmaxOut
                DO I=1,Ibmaxperjb(j)
                   gPresOut(i,k,j)= 0.5_r8 * (gPresaux(i,k,k)+gPresaux(i,K+1,j))
                END DO
             END DO
          ELSE
             ! Phillips interpolation over two interfaces
             DO k=1,KmaxOut
                DO I=1,Ibmaxperjb(j)
                   gPresOut(i,k,j)= 0.5_r8 * (gPresaux(i,k,j)+gPresaux(i,K+1,j))
                   !gPresOut(i,k,j)= ((gPresaux(i,k,j)**RoCp1-gPresaux(i,k+1,j)**RoCp1)/ &
                   !                (RoCp1*(gPresaux(i,k,j)-gPresaux(i,k+1,j))))**RoCpR
                END DO
             END DO
          ENDIF
       END DO
...
END SUBROUTINE GDAStoGANL
  ...
  IMPLICIT NONE
    ...
    IF ( GDASOnly) THEN
       ...
       IF(myid.EQ.0)  WRITE (UNIT=nfprt, FMT='(7F15.6)') DelSigmaInp
       IF(myid.EQ.0)  WRITE (UNIT=nfprt, FMT='(/,A)')  ' SigInterInp:'
       IF(myid.EQ.0)  WRITE (UNIT=nfprt, FMT='(7F15.6)') SigInterInp
       IF(myid.EQ.0)  WRITE (UNIT=nfprt, FMT='(/,A)')  ' SigLayerInp:'
       IF(myid.EQ.0)  WRITE (UNIT=nfprt, FMT='(7F15.6)') SigLayerInp
    ELSE
...
END SUBROUTINE GetGrADSInp
  ...
  SUBROUTINE GetGrADSOut()
    ...
    IF (myid.EQ.0) THEN
       ...
       IF ( IDVCInp == 0_i4 .OR. IDVCInp == 1_i4 ) THEN
          ! Sigma case, calculate pressure for each layer directly
          WRITE (UNIT=nfctl, FMT='(6F10.3)') 1000.0_r8*SigLayerOut(1:KmaxOut)
       ELSE IF ( IDVCInp == 2_i4 ) THEN
          ! Sigma-p case, first calculate pressure for each interface and then
          ! calculate pressure on each layer according to vertical structure
         WRITE (UNIT=nfctl, FMT='(6F10.3)') SigInterOut(1:KmaxOut)+1000.0_r8*SigLayerOut(1:KmaxOut)
       END IF
       WRITE (UNIT=nfctl, FMT='(3A)') 'tdef 1 linear ', Tdef,' 1dy'
       IF (NTracers == 1) THEN
...
SUBROUTINE GetGrADSInp_GDAS
  ...
  IF (myid.eq.0) THEN
    ...
    IF ( IDVCInp == 0_i4 .OR. IDVCInp == 1_i4 ) THEN
       ! Sigma case, calculate pressure for each layer directly
       WRITE (UNIT=nfctl, FMT='(6F10.3)') 1000.0_r8*SigLInp(1:KmaxInp)
    ELSE IF ( IDVCInp == 2_i4 ) THEN
       ! Sigma-p case, first calculate pressure for each interface and then
       ! calculate pressure on each layer according to vertical structure
      WRITE (UNIT=nfctl, FMT='(6F10.3)') SigIInp(1:KmaxInp)+1000.0_r8*SigLInp(1:KmaxInp)
    END IF
    WRITE (UNIT=nfctl, FMT='(3A)') 'tdef 1 linear ', Tdef,' 1dy'
    IF (NTracers == 1) THEN
