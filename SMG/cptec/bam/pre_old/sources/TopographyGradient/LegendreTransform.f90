MODULE LegendreTransform
  ...
  USE InputParameters, ONLY : r8, EMRad1, EMRad2, EMRad12, nferr, &
  ...
  PUBLIC :: CreateLegTrans, CreateSpectralRep, CreateGaussRep, &
            Spec2Four, Four2Spec, SplitTrans, DivVor, gLats, ColRad, snnp1, la0

  INTEGER, ALLOCATABLE, DIMENSION (:,:) :: la0, la1

  REAL (KIND=r8), ALLOCATABLE, DIMENSION (:) :: eps, snnp1

  REAL (KIND=r8), ALLOCATABLE, DIMENSION (:) :: ColRad, rCs2, Wgt, gLats

  REAL (KIND=r8), ALLOCATABLE, DIMENSION (:,:) :: LegS2F, LegExtS2F, LegDerS2F

  REAL (KIND=r8), ALLOCATABLE, DIMENSION (:,:) :: LegF2S, LegExtF2S, LegDerNS, LegDerEW
...
CONTAINS
  ...
  SUBROUTINE GaussianLatitudes ()
    ...
    INTEGER :: j
    REAL (KIND=r8) :: Epsil
    REAL (KIND=r8) :: rad, scale, dgColIn, gCol, dgCol, p2, p1
  ...
  SUBROUTINE CreateLegTrans ()
    ...
    ALLOCATE (LegExtF2S(Mnwv3, jMaxHf))
  ...
  SUBROUTINE LegPols()
   ...
   IMPLICIT NONE
    ...
    DO j=1,jMaxHf
       ...
       END DO
       DO mn=1,Mnwv3
          LegExtF2S(mn,j)=LegExtS2F(mn,j)*wgt(j)
       END DO
  ...
  SUBROUTINE Four2Spec2D (Four, Spec)
    ...
    IF (extended) THEN
       CALL SumFour (Mend2, Mend1, Mnwv3, f1, f2, jMaxHf, f3, &
                     Spec, LegExtF2S, Four, LenDiagExt, LastPrevDiagExt)
    ELSE
  ...
  SUBROUTINE SumDivVor (Nmax, Mmax, Mnwv, Imax, Jmax, Jmaxhf, Kmax, &
                        U, V, Div, Vor, LegNS, LegEW, Len, LastPrev)
    ...
    REAL (KIND=r8), INTENT(IN) :: U(Imax,Jmax,Kmax)  ! full fourier field
    REAL (KIND=r8), INTENT(IN) :: V(Imax,Jmax,Kmax)  ! full fourier field
    REAL (KIND=r8), INTENT(IN) :: LegNS(Mnwv,Jmaxhf) ! associated legendre function
    REAL (KIND=r8), INTENT(IN) :: LegEW(Mnwv,Jmaxhf) ! associated legendre function

    REAL (KIND=r8), INTENT(OUT) :: Div(Mnwv,Kmax) ! spectral field
    REAL (KIND=r8), INTENT(OUT) :: Vor(Mnwv,Kmax) ! spectral field

    INTEGER :: j, jj, k, ele, diag

    REAL (KIND=r8), DIMENSION (2*Mmax,Jmaxhf,Kmax) :: &
                    Vs, Vst, Vn, Vnt, Us, Ust, Un, Unt
    ...
    DO k=1,Kmax
       DO j=1,Jmaxhf
          jj=Jmax-j+1
          DO ele=1,2*Mmax
             Un(ele,j,k)=U(ele,j,k)+U(ele,jj,k)
             Us(ele,j,k)=U(ele,j,k)-U(ele,jj,k)
             Vn(ele,j,k)=V(ele,j,k)+V(ele,jj,k)
             Vs(ele,j,k)=V(ele,j,k)-V(ele,jj,k)
          END DO
       END DO
    END DO
    ...
    DO k=1,Kmax
       DO j=1,Jmaxhf
          DO ele=1,2*Mmax,2
             Unt(ele,j,k)=Un(ele+1,j,k)
             Unt(ele+1,j,k)=-Un(ele,j,k)
             Ust(ele,j,k)=Us(ele+1,j,k)
             Ust(ele+1,j,k)=-Us(ele,j,k)
             Vnt(ele,j,k)=-Vn(ele+1,j,k)
             Vnt(ele+1,j,k)= Vn(ele,j,k)
             Vst(ele,j,k)=-Vs(ele+1,j,k)
             Vst(ele+1,j,k)= Vs(ele,j,k)
          END DO
       END DO
    END DO
    ...
    DO k=1,Kmax
       DO j=1,Jmaxhf
          DO diag=1,Nmax,2
             DO ele=1,Len(diag)
                Div(ele+LastPrev(diag),k)=Div(ele+LastPrev(diag),k)- &
                     Vs (ele,j,k)*LegNS(ele+LastPrev(diag),j)- &
                     Unt(ele,j,k)*LegEW(ele+LastPrev(diag),j) 
                Vor(ele+LastPrev(diag),k)=Vor(ele+LastPrev(diag),k)+ &
                     Us (ele,j,k)*LegNS(ele+LastPrev(diag),j) + &
                     Vnt(ele,j,k)*LegEW(ele+LastPrev(diag),j) 
             END DO
          END DO
       END DO
    END DO
    ...
    DO k=1,Kmax
       DO j=1,Jmaxhf
          DO diag=2,Nmax,2
             DO ele=1,Len(diag)
                Div(ele+LastPrev(diag),k)=Div(ele+LastPrev(diag),k)- &
                     Vn (ele,j,k)*LegNS(ele+LastPrev(diag),j)- &
                     Ust(ele,j,k)*LegEW(ele+LastPrev(diag),j) 
                Vor(ele+LastPrev(diag),k)=Vor(ele+LastPrev(diag),k)+ &
                     Un (ele,j,k)*LegNS(ele+LastPrev(diag),j)+ &
                     Vst(ele,j,k)*LegEW(ele+LastPrev(diag),j) 
             END DO
          END DO
       END DO
    END DO

  END SUBROUTINE SumDivVor


  SUBROUTINE DivVor (U, V, Div, Vor)

    IMPLICIT NONE

    REAL (KIND=r8), INTENT(IN) :: U(:,:,:) ! full fourier field
    REAL (KIND=r8), INTENT(IN) :: V(:,:,:) ! full fourier field

    REAL (KIND=r8), INTENT(OUT) :: Div(:,:) ! spectral field
    REAL (KIND=r8), INTENT(OUT) :: Vor(:,:) ! spectral field

    INTEGER :: f1, f2, f3

    CHARACTER (LEN=17) :: h="**(DivVor)**"
    ...
    f1=SIZE(U,1); f2=SIZE(U,2); f3=SIZE(U,3)

    CALL SumDivVor (Mend1, Mend1, Mnwv2, f1, f2, Jmaxhf, f3, &
                    U, V, Div, Vor, LegDerNS, LegDerEW, LenDiag, LastPrevDiag)

  END SUBROUTINE DivVor
