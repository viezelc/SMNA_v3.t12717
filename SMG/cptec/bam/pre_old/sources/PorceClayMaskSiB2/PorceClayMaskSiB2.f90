...
PROGRAM PorceClayMaskSiB2
   ...
   INTEGER, DIMENSION (:,:), ALLOCATABLE :: PorceClayMaskSiB2In
   INTEGER, DIMENSION (:,:), ALLOCATABLE :: PorceClayMaskSiB2Out, VegMaskSave
   INTEGER, DIMENSION (:,:), ALLOCATABLE :: LandSeaMask, LSMaskSave
   INTEGER, DIMENSION (:,:), ALLOCATABLE :: PorceClayMaskSiB2Input
   INTEGER, DIMENSION (:,:), ALLOCATABLE :: PorceClayMaskSiB2Output
   ...
   ALLOCATE (PorceClayMaskSiB2Input(Idim,Jdim))
   ALLOCATE (PorceClayMaskSiB2Output(Imax,Jmax))
   ALLOCATE (PorceClayMaskSiB2In(Idim,Jdim))
   ALLOCATE (PorceClayMaskSiB2Out(Imax,Jmax), VegMaskSave(Imax,Jmax))
   ...
   ! Read In Input PorceClayMaskSiB2
   INQUIRE (IOLENGTH=LRecIn) PorceClayMaskSiB2In
   ...
   IF (ios /= 0) THEN
      ...
      STOP  ' ** (Error) **'
   END IF
   READ  (UNIT=nfcvm, REC=1) PorceClayMaskSiB2In
   CLOSE (UNIT=nfcvm)

   ! Interpolate Input Regular PorceClayMaskSiB2 To Gaussian Grid Output
   PorceClayMaskSiB2Input=INT(PorceClayMaskSiB2In)
   CALL DoAreaIntegerInterp (PorceClayMaskSiB2Input(:,:), PorceClayMaskSiB2Output(:,:))
   DO j=1,jMax
      DO i=1,iMax
         IF(LSMaskSave(i,j) /= 0 .AND.PorceClayMaskSiB2Output(i,j) ==0.0 )PorceClayMaskSiB2Output(i,j)=60
      END DO
   END DO

   VegMaskSave=INT(PorceClayMaskSiB2Output)
   
   ! Fix Problems on Vegetation Mask and Land Sea Mask
   CALL VegetationMaskCheck ()

   PorceClayMaskSiB2Out=INT(PorceClayMaskSiB2Output)
 
   ! Write Out Adjusted Interpolated PorceClayMaskSiB2
   INQUIRE (IOLENGTH=LRecOut) PorceClayMaskSiB2Out
   ...
   IF (ios /= 0) THEN
      ...
      STOP  ' ** (Error) **'
   END IF
   WRITE (UNIT=nfvgm, REC=1) PorceClayMaskSiB2Out
   CLOSE (UNIT=nfvgm)
   ...
   IF (GrADS) THEN
      ...
      Gad(:,:)=REAL(PorceClayMaskSiB2Out(:,:),r4)
...
PRINT *, "*** PorceClayMaskSiB2 ENDS NORMALLY ***"
...
SUBROUTINE VegetationMaskCheck ()
   ...
   10 CONTINUE
      DO j=1,Jmax
         DO i=1,Imax
            IF (LandSeaMask(i,j) /= 0 .AND. PorceClayMaskSiB2Output(i,j) == Undef) THEN
               IF (im == 0) THEN
                  im=Imax
               ELSE
                  im=i-1
              ...
              jm=j-1
               ...
               IF (LandSeaMask(im,jm) /= 0) THEN
                  k1=PorceClayMaskSiB2Output(im,jm)
               ELSE
                  k1=0
               END IF
               IF (LandSeaMask(i,jm) /= 0) THEN
                  k2=PorceClayMaskSiB2Output(i,jm)
               ELSE
                  k2=0
               END IF
               ip=max(min(Imax,ip),1)
               IF (LandSeaMask(ip,jm) /= 0) THEN
                  k3=PorceClayMaskSiB2Output(ip,jm)
               ELSE
                  k3=0
               END IF
               IF (LandSeaMask(im,j) /= 0) THEN
                  k4=PorceClayMaskSiB2Output(im,j)
               ELSE
                  k4=0
               END IF
               IF (LandSeaMask(ip,j) /= 0) THEN
                  k5=PorceClayMaskSiB2Output(ip,j)
               ELSE
                  k5=0
               END IF
               jp=max(min(Jmax,jp),1)
               IF (LandSeaMask(im,jp) /= 0) THEN
                  k6=PorceClayMaskSiB2Output(im,jp)
               ELSE
                  k6=0
               END IF
               IF (LandSeaMask(i,jp) /= 0) THEN
                  k7=PorceClayMaskSiB2Output(i,jp)
               ELSE
                  k7=0
               END IF
               IF (LandSeaMask(ip,jp) /= 0) THEN
                  k8=PorceClayMaskSiB2Output(ip,jp)
               ELSE
                  k8=0
               END IF
               ...
               PorceClayMaskSiB2Output(i,j)=kk
               WRITE (UNIT=*, FMT='(A,2I8,A,I8)') &
                     ' Undefined Location i, j = ', i, j, &
                     ' Filled with Nearby Value = ', kk
             END IF

             IF (LandSeaMask(i,j) == 0) PorceClayMaskSiB2Output(i,j)=Undef
...
SUBROUTINE VegetationMaskFix ()
   ...
   WRITE (UNIT=*, FMT='(A,2I8,/,A,/,(11X,3I8))') &
         ...
         ' With no Unambigous Nearby Values.  Local Area:', &
           k1, k2, k3, k4, PorceClayMaskSiB2Output(i,j), k5, k6, k7, k8
