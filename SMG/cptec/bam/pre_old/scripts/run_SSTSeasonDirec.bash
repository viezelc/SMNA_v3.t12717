...
cat <<EOT0 > ${direxe}/${varname}.nml
 &InputDim
  ...
  nMonthly =${nMonthly},  ! Number of monthly before persistida
  FirstYear=${FirstYear}, ! Initial Year of the temporal serie ( sst ) 1870 01 16 12
  LastYear =${LastYear},  ! Last    Year of the temporal serie ( sst )    2017 12
  FirstYearClim =1980,    ! Initial Year to sst climatology computation  1870 01 16 12
  LastYearClim  =2010,    ! Last    Year to sst climatology computation     2017 12
...
#PBS -A ${QUOTA}
#PBS -l nodes=${cpu_mpi}:ppn=${cpu_node}
#PBS -V
#PBS -S /bin/bash
#PBS -N $RES
#PBS -q ${AUX_QUEUE}
...
it=2
while [ ${it} -gt 0 ];do
it=`qstat @sdb | grep $USER | grep $RES | wc -l`
done
