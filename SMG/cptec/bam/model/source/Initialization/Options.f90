...
MODULE Options
  ...
  CHARACTER(LEN=6)  , PUBLIC           :: NMSST='sstaoi'
  ...
  LOGICAL           , PUBLIC           :: FIELDGAUS=.FALSE.
  LOGICAL           , PUBLIC           :: FILELIST=.FALSE.
  CHARACTER(LEN=5)  , PUBLIC           :: StrFormat=''   ! Get NCEP Analysis and dumping to 'old' format or 'new' format
  ...
  CHARACTER(LEN=200), PUBLIC           :: dirRstInput
...
!tar end 
  ...
  LOGICAL           , PUBLIC           :: grid_difus=.FALSE.
...
! REAL(KIND=r8)     , PUBLIC           :: filtb= (1.0_r8-filta)*0.5_r8
  ...
  REAL(KIND=r8)     , PUBLIC           :: alfa    =1! 0.53_r8
...
!tar end    
  ...
  REAL(KIND=r8)     , PUBLIC           :: eps_sic=1.00_r8! Factor for semi-implicit correction
  ...
  INTEGER           , PUBLIC           :: q_order=2  !2*q  th order hyper diffusion 
  REAL(KIND=r8)     , PUBLIC           :: Gamma_Factor=6.25e-3_r8 !GammaFactor =1.15e-3  --> TQ0062  non-dimensional empirical factors
                                                                  !GammaFactor =6.25e-3  --> TQ0299  non-dimensional empirical factors
                                                                  !GammaFactor =6.25e-3  --> TQ0126  non-dimensional empirical factors
...
CONTAINS
  ...
  SUBROUTINE ReadNameList()
    ...
    NAMELIST /MODEL_RES/trunc,vert,dt,idatei,idatew,idatef,nmsst,SetBCCte,NMCO2FX,NMSNOW,NMSOILM,&
         dhfct,dhres,dhdhn,nhdhn,dhext,nhext,dogrh,StrFormat,FIELDGAUS,FILELIST,&
         doprc,prefx,prefy,table,path_in,dirfNameOutput, dirRstInput, dirRstOutput

    NAMELIST /MODEL_IN/slagr,slhum,grid_difus,microphys,SL_twotime_scheme,nlnminit,diabatic, &
         eigeninit,intcosz,Model1D,mgiven,gaussgiven,reducedGrid, &
    ...
    NAMELIST /PHYSPROC/UNIFIED,rootmode,iswrad,ilwrad,iccon,ilcon,iqdif, &
...
!    IF(microphys)ilcon='MIC'
    ...
    END IF
    CALL MsgOne(h, " Checking options ")
    CALL CheckOptions()
...
!    fNameCldOptLW
    ...
    ! SEA SURAFACE TEMPERATURE

    IF (ifsst.EQ.0) THEN
       CALL MsgOne(h, tab//"SST CONSTANT ifsst=0")
    ELSE
       CALL MsgOne(h, tab//"Reading global SST field from external file")
       IF (ifsst.EQ.-1) THEN
          CALL MsgOne(h, tab//"SST field is constant weather forcasting ifsst=-1")
       ELSEIF (ifsst.EQ.1) THEN
          CALL MsgOne(h, tab//"SST field is constant  ifsst=1")
       ELSEIF (ifsst.EQ.2) THEN
          CALL MsgOne(h, tab//"SST field is climatology ifsst=2")
       ELSEIF (ifsst.EQ.3) THEN
          CALL MsgOne(h, tab//"SST field pessitida ifsst=3")
       ELSEIF (ifsst.EQ.4) THEN
          CALL MsgOne(h, tab//"SST field interpolated monthly ifsst=4")
       ELSEIF (ifsst.EQ.5) THEN
          CALL MsgOne(h, tab//"SST field interpolated daily ifsst=5")
       ENDIF
    ENDIF
...
!       rsettov  = .FALSE.
       ...
       !ifsnw=0
       IF(SetBCCte == 1)THEN
          ifsnw=0!3 !PKUBOTA ASSIMILATION
       ELSE
          ifsnw=0 
       ENDIF
       ifalb=0
       IF(SetBCCte == 1)THEN
          ifslm=0!1 !PKUBOTA ASSIMILATION
       ELSE
          ifslm=0 
       ENDIF
       !ifslm=0
  ...
  SUBROUTINE setsst (c0,IDATEW)
    CHARACTER(LEN=*), INTENT(IN   ) :: c0
    INTEGER         , INTENT(IN   ) :: IDATEW(4)
    INTEGER           :: LFSST
    INTEGER           :: LFCO2FLX
    INTEGER           :: LFNDVI
    INTEGER           :: LWSST
    ...
    IF(SetBCCte == 1)THEN
       IF (TRIM(START)  == 'warm') THEN
          WRITE(LABELC,'(i4.4,3i2.2)')IDATEW(4),IDATEW(3),IDATEW(2) !PKUBOTA ASSIMILATION
       ELSE
          WRITE(LABELC,'(i4.4,2i2.2)')idate(4),idate(2),idate(3)
       END IF
    ...
    IF ( TRIM(NMSST) == 'sstwkl' ) THEN
       fNameSSTAOI=TRIM(path_in1)//'SSTWeekly'//LABELC//'.G'//TRIM(c0)
...
!_____________________________________________________________________    
    ...
    IF(LEN(TRIM(NMCO2FX)) == 0 )THEN
       CALL FatalError(h//" NMCO2FX file ("//TRIM(NMCO2FX)//") not set")
    END IF
    ...
    ELSE IF ( TRIM(NMCO2FX) == 'co2flxwkd' ) THEN
       fNameCO2FLX=TRIM(path_in1)//'FLUXCO2Clima'//LABELS//'.'//TRIM(TRC)
...
!_____________________________________________________________________    
    ...
    IF(SetBCCte == 1)THEN
       IF (TRIM(START)  == 'warm') THEN
          IF ( LFSST == -1 ) THEN
             LWSST=LFSST  !PKUBOTA ASSIMILATION
          ELSE
             LWSST=LFSST
          END IF
       END IF
...
!tar end
    ...
    IF (ifozone >=1 .and. ifozone <=4 ) THEN
       ...
       ELSEIF (ifozone == 2) THEN ! clim interpol
          fNameOzone  = TRIM(path_in1)//'OZON'//TRIM(PREFY)//'CLIMATOLOGY'//'.grd.'//'G'//TRIM(c0)//TRIM(LV)
  ...
  IMPLICIT NONE
    ...
    ELSEIF ( TRIM(START) == 'warm' ) THEN
       ...
       EXTH='F.unf'
       EXDH='F.dir'
       fNameInput0=TRIM(dirRstInput)//'/'//'GFCT'//TRIM(PREFX)//LABELC//LABELC//EXTW//'.'//TRIM(TRCG)//TRIM(LV)//'.outmdt'
       fNameInput1=TRIM(dirRstInput)//'/'//'GFCT'//TRIM(PREFX)//LABELC//LABELC//EXTW//'.'//TRIM(TRCG)//TRIM(LV)//'.outatt'
    END IF
  ...
  SUBROUTINE FileNameRest(PRC,OPT)
   IMPLICIT NONE
    ...
    IF (OPT==1) THEN
        FNameRestInput1=TRIM(dirRstInput)//"/"//"GFCT"//TRIM(PREFX)//labelc//labelc//&
            &EXTW//'.'//TRIM(TruncLev)//'.outmdt'//TRIM(PRC)

        FNameRestInput2=TRIM(dirRstInput)//"/"//"GFCT"//TRIM(PREFX)//labelc//labelc//&
            &EXTW//'.'//TRIM(TruncLev)//'.outatt'//TRIM(PRC)

        FNameConvClInp0=TRIM(dirRstInput)//"/"//"GFCT"//TRIM(PREFX)//labelc//labelc//&
            &EXTW//'.'//TRIM(TruncLev)//'.convcl'//TRIM(PRC)

        FNameSibPrgInp0=TRIM(dirRstInput)//"/"//"GFCT"//TRIM(PREFX)//labelc//labelc//&
  ...
  SUBROUTINE FileNameRest2(PRC,OPT,REALTIME)
   IMPLICIT NONE
    ...
    IF (OPT==1) THEN
        FNameRestInput1=TRIM(dirRstInput)//"/"//"GFCT"//TRIM(PREFX)//labeli//labeli//&
            &EXTW//'.'//TRIM(TruncLev)//'.outmdt'//TRIM(PRC)

        FNameRestInput2=TRIM(dirRstInput)//"/"//"GFCT"//TRIM(PREFX)//labeli//labeli//&
            &EXTW//'.'//TRIM(TruncLev)//'.outatt'//TRIM(PRC)

        FNameConvClInp0=TRIM(dirRstInput)//"/"//"GFCT"//TRIM(PREFX)//labeli//labeli//&
            &EXTW//'.'//TRIM(TruncLev)//'.convcl'//TRIM(PRC)

        FNameSibPrgInp0=TRIM(dirRstInput)//"/"//"GFCT"//TRIM(PREFX)//labeli//labeli//&
            &EXTW//'.'//TRIM(TruncLev)//'.sibprg'//TRIM(PRC)
    ELSE
        FNameSibPrgOut1=TRIM(dirRstOutput)//"/"//"GFCT"//TRIM(PREFX)//labelc//labelc//&
             &EXTW//'.'//TRIM(TruncLev)//'.sibprg'//TRIM(PRC)
  ...
  SUBROUTINE SetOutPut (tod1,idate)
    ...
    INTEGER  :: idays
    INTEGER  :: imont
    INTEGER  :: iyear,ierr,yi,mi,di,hi,LenYearbyDay
    REAL(KIND=r8)  :: xday
    ...
    ELSE
       ...
       DO jdt=1,maxtim
          ...
          CALL jull_options(iyear,imont,idays,ihour,xday,LenYearbyDay)
  ...
  SUBROUTINE SetOutPutDHN (tod1,idate)
    ...
    INTEGER  :: l
    INTEGER  :: maxtfm2
    INTEGER  :: jdt,yi,mi,di,hi,LenYearbyDay
    INTEGER  :: ihour
    INTEGER  :: idays
    INTEGER  :: imont
    INTEGER  :: iyear
  ...
  CHARACTER(LEN=10) FUNCTION CalDate(labelc,opt)
    ...
    INTEGER :: ihour
    REAL(KIND=r8) :: xday
    INTEGER :: lenmon,yi,mi,di,hi,LenYearbyDay
    ...
    IF(opt=='-')THEN
       ...
       IF(imonth-1 == 2 .AND. LenYearbyDay == 366)nday=29

       lenmon=lenMonth(imonth)
       IF(imonth == 2 .AND. LenYearbyDay == 366)lenmon=29
       iday=iday-nday
       IF (iday < 1) THEN
          imonth=imonth-1
          IF(imonth <1) THEN
    ...
    ELSE IF(opt=='+')THEN
       ...
       IF(imonth == 2 .AND. LenYearbyDay == 366)nday=29
  ...
  SUBROUTINE Caldat(julian,mm,id,iyyy)
     ...
     INTEGER :: ioptn, iday, month, iyear,julianday
     ...
     month=mm
     iyear=iyyy
     CALL calndr (ioptn, iday, month, iyear, julianday)
     mm=month
     id=iday
     iyyy= iyear
  ...
  INTEGER FUNCTION julday(mm,id,iyyy)
   ...
   INTEGER :: jy, jm, ja,ioptn,  day, mon, year,idayct2
   IF (iyyy.EQ.0) THEN
  ...
  SUBROUTINE jull_options(yi,mi,di,hi,xday,LenYearbyDay)
    ...
    INTEGER                :: monl(12)
    INTEGER                :: monday(12)
    INTEGER                :: m,idayct2,day, mon, year,ioptn
  ...
  SUBROUTINE jull(yi,mi,di,hi,tod,xday,nday2y,LenYearbyDay)
