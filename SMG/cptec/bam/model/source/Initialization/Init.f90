...
CONTAINS
  ...
  SUBROUTINE InitAll (trunc, vert, reducedGrid, linearGrid, mgiven, gaussgiven, &
       ct_in, cq_in, a_in, b_in, dk_in, tk_in ,dt)
    ...
    REAL(KIND=r8) :: sl(vert) ,si(vert+1),bk(vert)
    REAL(KIND=r8) :: f0
    INTEGER :: wavesFactor
    INTEGER :: nLon
    INTEGER :: nLat, left
    ...
    CALL InitVerSizes (a_in, b_in)

    nls=0
    DO k=1,kMax
       sl(k) = 0.5_r8 * ( (a_in(k)+a_in(k+1))/1.0e5_r8 + b_in(k)+b_in(k+1) )
       bk(k) = 0.5_r8 * (b_in(k)+b_in(k+1) )
       IF (sl(k)<= Sigma_Estrat) nls=nls+1
    END DO
    IF (nls == 0) nls=1
    nlcs=kMax+2
    si(kMax+1) = 1.00_r8
    DO k=kMax,2,-1
      si(k) = 0.5_r8*(sl(k-1) + sl(k)) 
    END DO
    si(1) = 0.00_r8

   CALL Init_ct_cq (kMax, si(kMax+1:1:-1), sl(kMax:1:-1), ct_in(kMax:1:-1), cq_in(kMax:1:-1))

   !f0=1.0_r8
   DO k=1,kMax
      PRINT*,ct_in(k),cq_in(k)
   END DO
...
!PKUBOTA    cq_in = 0._r8
    ...
    CALL InitSemiImpl()
    CALL InitImplDifu( ct_in, cq_in, dk_in, tk_in,dt)
