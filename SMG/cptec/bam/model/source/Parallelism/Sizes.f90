...
MODULE Sizes   ! Version 0 of Nov 25th, 2001
  ...
  REAL(KIND=r8), ALLOCATABLE :: eta(:)     ! Hybrid vertical coordinate
  REAL(KIND=r8), ALLOCATABLE :: del_eta(:) ! Hybrid vertical coordinate spacing
  REAL(KIND=r8), ALLOCATABLE :: eta_i(:)   ! Hybrid vertical coordinate (cell interfaces)
  REAL(KIND=r8), ALLOCATABLE :: del_eta_i(:) ! Hybrid vertical coordinate spacing from interfaces
  REAL(KIND=r8), ALLOCATABLE :: a_hybr(:)  ! Constant for defining Hybrid coordinate
  REAL(KIND=r8), ALLOCATABLE :: b_hybr(:)  ! Constant for defining Hybrid coordinate
  REAL(KIND=r8), ALLOCATABLE :: c_hybr(:)  ! coefficient related to hybrid coordinate
  REAL(KIND=r8), ALLOCATABLE :: delb(:)    ! layer differences (in b_hybr)
  REAL(KIND=r8), ALLOCATABLE :: p_r(:)     ! p reference levels
  REAL(KIND=r8), ALLOCATABLE :: delp_r(:)  ! differences between p ref levels
  REAL(KIND=r8), ALLOCATABLE :: alpha_r(:) ! coefficients for half level correction of geopotential
  REAL(KIND=r8), ALLOCATABLE :: rpi_r(:)   ! log ratio between ref p levels
...
CONTAINS
  ...
  SUBROUTINE SpectralDomainDecomp()
    ...
    havesurf = .FALSE.
!
!   Now, with indexing from top to bottom, the last group has the surface fields
!
!   IF (mygroup_four.eq.ngroups_four) havesurf = .TRUE.
...
!   mylastmn_si = myfirstmn_si + MNMax_si - 1
  ...
  SUBROUTINE InitVerSizes (a_in, b_in)

    REAL(KIND=r8), INTENT(IN) :: a_in(:)
    REAL(KIND=r8), INTENT(IN) :: b_in(:)
    ...
    kMaxsl = SIZE(a_in)
    IF (kMaxsl /= kMax+1) STOP ' Error in InitVerSizes: kMaxsl=SIZE(sl) /= kMax+1 '

    ALLOCATE(delb(kMax))
    ALLOCATE(a_hybr(kMax+1))
    ALLOCATE(b_hybr(kMax+1))
    ALLOCATE(c_hybr(kMax))
    ALLOCATE(eta(kMax))
    ALLOCATE(del_eta(kMax-1))
    ALLOCATE(eta_i(kMax+1))
    ALLOCATE(del_eta_i(kMax))
    ALLOCATE(p_r(kMax+1))
    ALLOCATE(delp_r(kMax))
    ALLOCATE(rpi_r(kMax))
    ALLOCATE(alpha_r(kMax))
    ...
    END DO
    !     
    !     compute p reference levels
    !
    DO k=1, kMax+1
       p_r(k) = a_hybr(k) + psref * b_hybr(k) 
       eta_i(k) = a_hybr(k)/p000+b_hybr(k)
    END DO
    DO k=1, kMax
       delp_r(k) = p_r(k+1) - p_r(k)
       c_hybr(k) = a_hybr(k+1) * b_hybr(k) - a_hybr(k) * b_hybr(k+1)
       eta(k) = 0.5_r8 * (eta_i(k)+eta_i(k+1))
       del_eta_i(k) = eta_i(k+1) - eta_i(k)
    END DO
    alpha_r(1) = log(2._r8)
    rpi_r(1) = 1.   !!   not used
    DO k=2, kMax
       del_eta(k-1) = eta(k) - eta(k-1)
       rpi_r(k) = log(p_r(k+1)/p_r(k))
       alpha_r(k) = 1._r8-p_r(k)*rpi_r(k)/delp_r(k)
    END DO

    ! Write hybrid levels and related arrays on model text output
    CALL MsgOne(h," Model hybrid levels")
    IF (myId.eq.0) THEN
       WRITE(*,*) ' a, b, delb, p_r, delp_r, rpi_r, alpha_r '
       DO k=1,kmax
          WRITE(*,'(i3,2x,7(F13.6,2x))') k, &
          a_hybr(k),b_hybr(k),delb(k),p_r(k),delp_r(k),rpi_r(k),alpha_r(k)
       ENDDO
       k=kmax+1
          WRITE(*,'(i3,2x,3(F13.6,2x))') k, &
          a_hybr(k),b_hybr(k),p_r(k)
    ENDIF
