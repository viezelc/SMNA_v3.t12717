MODULE PhysicalFunctions
 IMPLICIT NONE
  ...
  REAL(KIND=r8) :: psaditmk  (-1:150,-1:150) 
...
CONTAINS
  ...
  !p-> pressure in mbar
    ...
    DO i=1,nCols
        ght(i,1)=0.5_r8*(tmk(i,1)+tsfc(i))*(gasr/grav)*(PSFC(i)-prs(i,1))/PSFC(i)
    ...
    DO i = 1,nCols
       ...
       ELSE                           !! 2D case
          ...
          ethmax = -1.0_r8
          DO k = 1, kMax
             IF ( ght(i,k)-ter(i) .LT. 1500.0_r8 ) THEN
                q        = MIN(MAX(qvp(i,k),1.e-12_r8),0.03_r8)
                temp     = MAX(MIN(tmk(i,k),tsfc(i)),180.0_r8)
                pressure = prs(i,k)
...
34           CONTINUE
          END DO
35        CONTINUE
          IF(totprs ==  0.0_r8)THEN
             q        = MAX(qvp(i,k),1.e-15_r8)
             qvppari  = MAX(qvp(i,k),1.e-15_r8)
             th       = tmk(i,k)*(PSFC(i)/pressure)**(GAMMA_RIP*(1.0_r8+GAMMAMD*q))
             tmkpari = th
          ELSE
             qvppari = MAX(totqvp/totprs,1.e-15_r8)
             tmkpari = (totthe/MAX(totprs,1.e-6_r8))*(prs(i,kpar1)/PSFC(i))**    &
...
!    REAL(KIND=r8)                :: tlcl
  ...
  REAL(KIND=r8) FUNCTION tonpsadiabat (thte,prs,PSFC)
    ...
    DO jtch = 1,150-4
       IF ( thte.GE.psadithte(jtch) .AND. thte.LT.psadithte(jtch+4) ) THEN
          jt = jtch
          GOTO 213
       END IF
    END DO
...
213 CONTINUE
    ...
    DO ipch = 1,150-4
       IF ( prs.LE.psadiprs(ipch) .AND. prs.GT.psadiprs(ipch+4) ) THEN
          ip = ipch
          GOTO 215
       END IF
    ENDDO
...
215 CONTINUE
    ...
    IF ( psaditmk(ip,jt  ).GT.1e9_r8 .OR. psaditmk(ip+1,jt  ).GT.1e9_r8 .OR.   &
       ...
       PRINT*, 'Prs and Thte probably unreasonable. prs,thte=',prs,thte,ip,jt,psaditmk(ip,jt  )
       tonpsadiabat = thte*(prs/PSFC)**GAMMA_RIP
       RETURN
    ENDIF
...
CONTAINS
  ...
  SUBROUTINE GET_PHI(&
                    ...
                    imask  , &!INTEGER(KIND=i8), INTENT(IN) :: imask  (ix)
                    TSK    , &!REAL(KIND=r8),    INTENT(in) :: TSK    (ix)
                    prsl   , &!REAL(KIND=r8),    INTENT(in) :: prsl   (ix,levs)
    ...
    INTEGER(KIND=i8), INTENT(IN) :: imask  (ix)
    REAL(KIND=r8),    INTENT(in) :: TSK    (ix)
    REAL(KIND=r8),    INTENT(in) :: prsl   (ix,levs)
    ...
    REAL(kind=r8) :: xr(ix,levs)
    REAL(KIND=r8) :: ze      (ix,levs),htdisp(ix)
    REAL(KIND=r8) :: zehalf  (ix,levs + 1)
    REAL(KIND=r8) :: DeltaP(ix,levs)
    REAL(KIND=r8) :: tv      (ix,levs)

    REAL(KIND=r8) :: delz    (ix,levs)
    REAL(KIND=r8) :: delzhalf(ix,levs + 1)

    REAL(kind=r8) :: tem
    REAL(kind=r8) :: dphib
    REAL(kind=r8) :: dphit
    REAL(kind=r8) :: dphi,rbyg
    REAL (kind=r8), PARAMETER :: zero=0.0
    REAL (KIND=r8), PARAMETER :: gasr  = 287.05_r8! gas constant of dry air        (j/kg/k)
    REAL (KIND=r8), PARAMETER :: grav  = 9.8e0_r8! gravity constant               (m/s**2)
    INTEGER       :: i, k
    ...
    ELSE                                   ! Not gc Virt Temp (Orig Joe)
       ...
       DO k=1,levs
          DO i=1,ix
             tv(i,k)=T(i,k)*(1.0_r8+con_fvirt*MAX(Q(i,k,1),zero))
          ENDDO
       ENDDO
       ze=0.0_r8
       zehalf=0.0_r8
       ...
       DO i=1,ix
          ...
          IF(imask(i) >1.0_r8)THEN
             delz    (i,1)=MAX((rbyg * (tv(i,1)+TSK(i))*0.5 - htdisp(i)),0.5_r8)*0.75_r8
             delzhalf(i,1)=MAX((rbyg * TSK(i)  - htdisp(i)),0.5_r8)*0.75_r8
          ELSE
             delz    (i,1)=MAX((rbyg * (tv(i,1)+TSK(i))*0.5 - htdisp(i)),0.5_r8)
             delzhalf(i,1)=MAX((rbyg * TSK(i)  - htdisp(i)),0.5_r8)
          END IF
          ze    (i,1)=MAX(0.0_r8,delz(i,1))
          zehalf(i,1)=0.0_r8!MAX(0.0_r8,delzhalf(i,1))
       END DO
       DO k=2,levs
         DO i=1,ix
            delzhalf(i,k)=0.5_r8*gasr*(tv(i,k-1)+tv(i,k))* &
                 LOG(prsi(i,k-1)/MAX(prsi(i,k),1e-12_r8))/grav
            zehalf(i,k)=zehalf(i,k-1)+ delzhalf(i,k)
         END DO
       END DO
       DO i=1,ix
          delzhalf(i,levs+1)=gasr*(tv(i,levs))* &
               LOG(prsi(i,levs)/(prsi(i,levs)*0.1_r8))/grav
          zehalf(i,levs+1)=zehalf(i,levs)+ delzhalf(i,levs+1)
       END DO
       DO k=2,levs
          DO i=1,ix
             delz(i,k)=0.5_r8*gasr*(tv(i,k-1)+tv(i,k))* &
                  LOG(prsl(i,k-1)/prsl(i,k))/grav
       ...
       DO k=1,levs+1
          DO i=1,ix
             phii(i,k) = zehalf(i,k)*grav
          END DO
       END DO
        DO k=1,levs
          DO i=1,ix
             phil(i,k) = ze(i,k)*grav
          END DO
       END DO
