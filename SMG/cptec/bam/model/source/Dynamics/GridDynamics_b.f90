...
CONTAINS
  ...
  SUBROUTINE GrpComp(&
       ...
       gphil  , gphii  , gicem  , gicet  , &
       gliqm  , gliqt  , gvarm  , gvart  )   
    !
    ...
    REAL(KIND=r8),    INTENT(OUT  ) :: gvdlnp (ibMax, kMax)
    ...
    REAL(KIND=r8),    INTENT(INOUT) :: gvdlnpm(ibMax, kMax)
    ...
    CALL tmp_lnp_tend(gtd,gvdlnp,gtmp,gdiv,gq,ps,tref,psint,adveps, &
               ...
               delb,zsint,slagr,psref,ibMax,kMax,ibLim,omg)
    !
    ...
    IF (dodia(nDiag_homtvu)) CALL updia (gv(:,kMax:1:-1)*gu(:,kMax:1:-1), nDiag_homtvu, latco)
    !     Vertical Zonal Momentum Transport
    IF (dodia(nDiag_vzmtwu)) CALL updia (omg(:,kMax:1:-1)*gu(:,kMax:1:-1),  nDiag_vzmtwu, latco)
    !     Vertical Meridional Momentum Transport
    IF (dodia(nDiag_vmmtwv)) CALL updia (omg(:,kMax:1:-1)*gv(:,kMax:1:-1),  nDiag_vmmtwv, latco)
    IF (dodia(nDiag_mshtvt) .OR. dodia(nDiag_zshtut) .OR. dodia(nDiag_vshtwt)) THEN
       ...
       IF (dodia(nDiag_mshtvt)) CALL updia (gv(:,kMax:1:-1)*gdt(:,kMax:1:-1),  nDiag_mshtvt, latco)
       !     Zonal Sensible Heat Transport
       IF (dodia(nDiag_zshtut)) CALL updia (gu(:,kMax:1:-1)*gdt(:,kMax:1:-1),  nDiag_zshtut, latco)
       !     Vertical Sensible Heat Transport
       IF (dodia(nDiag_vshtwt)) CALL updia (omg(:,kMax:1:-1)*gdt(:,kMax:1:-1), nDiag_vshtwt, latco)
    END IF
    !     Meridional Specific Humidity Transport
    IF (dodia(nDiag_mshtuq)) CALL updia (gv(:,kMax:1:-1)*gq(:,kMax:1:-1),   nDiag_mshtuq, latco)
    !     Zonal Specific Humidity Transport
    IF (dodia(nDiag_zshtuq)) CALL updia (gu(:,kMax:1:-1)*gq(:,kMax:1:-1),   nDiag_zshtuq, latco)
    !     Vertical Specific Humidity Transport
    IF (dodia(nDiag_vshtwq)) CALL updia (omg(:,kMax:1:-1)*gq(:,kMax:1:-1),  nDiag_vshtwq, latco)
    !
    ...
    IF (dodia(nDiag_dewptt)) THEN
       DO k=1,kMax
          ...
          END DO
       END DO
       CALL updia (dewpwtr,nDiag_dewptt,latco)
    END IF
    !
    !
    IF (.NOT. slagr .AND. .NOT. slhum) THEN
       ...
       !
    ENDIF
    IF(dodia(nDiag_divgxq)) CALL updia(psint(:,kMax:1:-1), nDiag_divgxq,latco)
    IF(dodia(nDiag_vmoadv)) CALL updia(divint(:,kMax:1:-1),nDiag_vmoadv,latco)
    IF(dodia(nDiag_omegav)) CALL updia(omg(:,kMax:1:-1),   nDiag_omegav,latco)
    !
    !
    !
    IF (IsGridHistoryOn()) THEN
    ...
    !
    !
    IF(dodia(nDiag_sigdot))CALL updia(gw(:,kMax:1:-1),nDiag_sigdot,latco)
    ...
    IF(TRIM(isimp).ne.'YES')THEN
          ...
          IF((nClass+nAeros)>0 .and. PRESENT(gvarm))THEN
             CALL DryPhysics & 
               (gzs, gtm, gqm, gum, gvm, ps, gyu, gyv, gtd, gqd, colrad, &
                ...
                jb, lonrad, glnpm, cos2d, intcosz, gicem, gicet, gliqm, gliqt, gvarm, gvart)
          ELSE
             CALL DryPhysics & 
               (gzs, gtm, gqm, gum, gvm, ps, gyu, gyv, gtd, gqd, colrad, &
                ifday , tod, gtmp, gq, omg, gprsl, gprsi, gphil, gphii, &
                jb, lonrad, glnpm, cos2d, intcosz, gicem, gicet, gliqm, gliqt)
          END IF
         ELSE
          CALL DryPhysics & 
               (gzs, gtm, gqm, gum, gvm, ps, gyu, gyv, gtd, gqd, colrad, &
       ...
       IF(dodia(nDiag_pwater))CALL updia(dewpwtr(:,kMax:1:-1) ,nDiag_pwater,latco)
       !
    END IF
    !
...
!         tendv(i,k) = tendv(i,k) - gasr * (av +apv(i)) - zsphi(i)
  ...
  SUBROUTINE tmp_lnp_tend(tend_t, tend_l, tmp, div, q, ps, tref, psint, adveps, &
    ...
    REAL(KIND=r8),    INTENT(INOUT) :: tend_l(ibMax,kMax)
    INTEGER :: i, k
    ...
    IF (slagr) THEN
       DO i=1,ibLim
          tend_l(i,1) = tend_l(i,1) - (divint(i,kmax)/ps(i)-eps_sic*s(i)/psref) - psint(i,kmax) + &
                                   zsint(i) / (gasr*tref)
       ENDDO
       DO k=kMax,1,-1
          DO i=1,ibLim
             tend_l(i,k) = tend_l(i,1) + adveps(i,k)
          ENDDO
       ENDDO
     ELSE
       DO i=1,ibLim
  ...
  SUBROUTINE tndtold(tdu, tdv, tdt, tdlnp, plam, pphi, zslam, zsphi, div, tlam, &
    ...
    REAL(KIND=r8),    INTENT(INOUT) :: tdlnp(ibMax,kMax)
    INTEGER :: i, k
    ...
    DO i=1,ibLim
       tdlnp(i,1) = tdlnp(i,1) - s * intl(i)
    ENDDO
    DO k=2,kMax
       tdlnp(1:ibLim,k) = tdlnp(1:ibLim,1)
    ENDDO
  ...
  SUBROUTINE AddTend(dt, nlnminit, jbFirst, jbLast, slhum)
    ...
    IF (.NOT.nlnminit) THEN
       ...
       DO jb = jbFirst, jbLast
          ...
          END DO
          DO ib = 1, ibMaxPerJB(jb)
             fgvdlnp(ib,1,jb) = fglnpm(ib,jb) + &
                  dt2 * ( fgvdlnp(ib,1,jb) + fgvdlnpm(ib,1,jb) )
          END DO
       END DO
    ...
    ELSE
       ...
       DO jb = jbFirst, jbLast
          ...
          END DO
          DO ib = 1, ibMaxPerJB(jb)
             fgvdlnp(ib,1,jb) = fgvdlnp(ib,1,jb) + 2.0_r8 * fgvdlnpm(ib,1,jb)
          END DO
       END DO
