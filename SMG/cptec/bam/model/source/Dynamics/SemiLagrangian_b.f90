...
!      Dumpgra
  ...
  REAL(KIND=r8), PUBLIC, ALLOCATABLE :: ueta   (:,:)
  REAL(KIND=r8), PUBLIC, ALLOCATABLE :: uetam  (:,:)
  ...
  REAL(KIND=r8), ALLOCATABLE :: etac(:,:)
  REAL(KIND=r8), ALLOCATABLE :: deletac(:,:)
...
CONTAINS
  SUBROUTINE SemiLagr(nit , delt, slagr, slhum)
    ...
    ! ----------------------------------
       IF (slagr) THEN
          ...
          fgvmean(:,j) = 0.0_r8
          DO k = 1, kmax
             fgumean(:,j) = fgumean(:,j) + fgu(:,k,j) * delb(k)
             fgvmean(:,j) = fgvmean(:,j) + fgv(:,k,j) * delb(k)
          ENDDO
       ENDIF
    ENDDO
    !
    ...
    DO
      ...
      ENDIF
      DO k=1,Kmax
        fgvdlnpm(:,k,j) = fglnpm(:,j) + fgzs(:,j) / (tref*gasr) & 
             + dt2 * fgvdlnpm(:,k,j) + delta * fgvdlnp(:,k,j)
      ENDDO
      fgqp(:,:,j) = delta * fgqd(:,:,j)
    ...
    DO
       ...
       ! ------------------------
          CALL Trajectory(ulonm(1,jb),ulatm(1,jb),uetam(1,jb),&
            ulon  , ulat  , ueta   , &
            ulondp, ulatdp, uetadp , &
            iloc  , jloc  , kloc   , &
            ibdim , jb    ,          &
            nit   , delta1 , ip, jn)           
       !
       ...
       IF (.not.slhum) THEN
        ...
        ELSE
          DO k=1,kmax
             DO i=1,ibdim
                fgqp(i,k,jb) = fint((k-1)*ibdim+i) + fgqp(i,k,jb)
             ENDDO
          ENDDO
       ENDIF
       IF (microphys) THEN
          CALL Interpcublin3d(fint  , fgicet , .TRUE. , &
               ulondp, ulatdp, uetadp, &
               iloc  , jloc  , kloc  , &
               ibdim ,  1.0_r8  ,.FALSE., &
               ip , jn, &
          ...
          DO k=1,kmax
             DO i=1,ibdim
                fgicep(i,k,jb) = fint((k-1)*ibdim+i) + fgicep(i,k,jb)
             ENDDO
          ENDDO
          CALL Interpcublin3d(fint  , fgliqt , .TRUE. , &
               ulondp, ulatdp, uetadp, &
               iloc  , jloc  , kloc  , &
               ibdim ,  1.0_r8  ,.FALSE., &
               ip , jn, &
          ...
          DO k=1,kmax
             DO i=1,ibdim
                fgliqp(i,k,jb) = fint((k-1)*ibdim+i) + fgliqp(i,k,jb)
             ENDDO
          ENDDO
          DO kk=1,nClass+nAeros
             CALL Interpcublin3d(fint  , fgvart(:,:,:,kk) , .TRUE. , &
                  ulondp, ulatdp, uetadp, &
                  iloc  , jloc  , kloc  , &
                  ibdim ,  1.0_r8  ,.FALSE., &
                  ip , jn, &
             ...
             DO k=1,kmax
                DO i=1,ibdim
                   fgvarp(i,k,jb,kk) = fint((k-1)*ibdim+i) + fgvarp(i,k,jb,kk)
                END DO
             END DO
          END DO
       ENDIF
       IF (slagr) THEN
          ...
          DO k=1,kmax
             ...
             ENDDO
          ENDDO
          CALL Interpcublin3d(fint  , fgvdlnpm , .TRUE.  , &
               ulondp, ulatdp, uetadp, &
               iloc  , jloc  , kloc  , &
               ibdim ,  1.0_r8  ,.FALSE., &
               ip , jn, &
          ...
          DO k=1,kmax
             DO i=1,ibdim
                fgvdlnp(i,k,jb) = delb(k) * (fint((k-1)*ibdim+i)  + delta * fgvdlnp(i,k,jb))
             ENDDO
          ENDDO
          DO k=2,kmax
              fgvdlnp(1:ibdim,1,jb) = fgvdlnp(1:ibdim,1,jb) + fgvdlnp(1:ibdim,k,jb)
          ENDDO
       ENDIF
       !
       ...
       IF (nscalars.gt.0) THEN
          ...
          IF (reducedGrid) klats = 4
          CALL Locate(.TRUE. ,.TRUE.  , &
               ulonm(1,jb), ulatm(1,jb), uetam(1,jb), &
               ulondp, ulatdp, uetadp , &
               iloc  , jloc  , kloc   , &
               ibdim , klats)
          log1 = .FALSE.
  ...
  SUBROUTINE SemiLagr_2tl(nit , delt, fa)
    ...
    ! ----------------------------------
       ...
       fgvmmean(:,j) = 0.0_r8
       DO k = 1, kmax
          fgummean(:,j) = fgummean(:,j) + fgum(:,k,j) * delb(k)
          fgvmmean(:,j) = fgvmmean(:,j) + fgvm(:,k,j) * delb(k)
       ENDDO
    ENDDO
    !
    ! Exchange Winds for overlapping areas
...
!      fgpsp(:,j) = fglnps(:,j) + fgzs(:,j) / (tref*gasr) & 
!           + delt * ( fgvdlnpm(:,j) + fgvdlnp(:,j) )
    ...
    DO
       ...
       ! compute departure points 
       ! ------------------------
       CALL Trajectory_2L(ulonm(1,jb),ulatm(1,jb),ueta(1,jb),&
            ulon(1,jb)  , ulat(1,jb)  , &
            ulondp, ulatdp, uetadp , &
            iloc  , jloc  , kloc   , &
            ibdim , jb    ,          &
...
!      fgvdlnp(1:ibdim,jb) = fint(1:ibdim) + dt2 * fgvdlnp(1:ibdim,jb)
  ...
  SUBROUTINE Trajectory_2L(        &
    ...
    REAL(KIND=r8)   , INTENT(INOUT) :: ueta  (ibdim,kmax)
    ...
    REAL(KIND=r8)   , INTENT(INOUT) :: uetadp(ibdim,kmax)
    !
    !  local variables 
    !
    INTEGER                        :: kit
    ...
    DO kit=1,nit
       ...
       CALL InterplG(&
            ...
            ibdim , ip , jn)
       !
       ! update eta level of mid-point of trajectory
       ! ---------------------------------------------
       !
       CALL Upeta_2L(ueta, fgw(1,1,jb), wint, delt, ibdim)
       !
       ! computes new eta location for interpolation
       ! ---------------------------------------------
       !
  ...
  SUBROUTINE Trajectory(        &
    ...
    REAL(KIND=r8)   , INTENT(INOUT) :: ulonm (ibdim,kmax)
    REAL(KIND=r8)   , INTENT(INOUT) :: ulatm (ibdim,kmax)
    REAL(KIND=r8)   , INTENT(INOUT) :: uetam (ibdim,kmax)
    REAL(KIND=r8)   , INTENT(INOUT) :: ulondp(ibdim,kmax,4)
    REAL(KIND=r8)   , INTENT(INOUT) :: ulatdp(ibdim,kmax)
    REAL(KIND=r8)   , INTENT(INOUT) :: uetadp(ibdim,kmax)
    !
    ...
    DO kit=1,nit
       ...
       ! ---------------------------------------------
       !
       CALL Upeta (ueta, uetam, wint, delt, ibdim, final)
       !
       ! computes new eta location for interpolation
       ! ---------------------------------------------
       !
  ...
  SUBROUTINE Upeta_2L (ueta, w, wm, delt, ibdim)
    !
    ! Upeta  : update eta upstream
    !
    !
    INTEGER, INTENT(IN   ) :: ibdim
    REAL(KIND=r8)   , INTENT(IN   ) :: w     (ibmax,kmax)
    REAL(KIND=r8)   , INTENT(IN   ) :: wm    (ibdim,kmax)
    REAL(KIND=r8)   , INTENT(OUT  ) :: ueta  (ibdim,kmax)
    REAL(KIND=r8)   , INTENT(IN   ) :: delt
    !
    !  local variables 
    !
    ...
    DO k=1,kmax
       DO i=1,ibdim
          ueta(i,k) = MIN(eta(kmax),MAX(eta(1),eta(k) - dth * (w(i,k)+wm(i,k))))
       ENDDO
    ENDDO
    !
  END SUBROUTINE Upeta_2L
  ...
  SUBROUTINE Upeta (ueta, uetam, w, delt, ibdim, final)
    !
    ! Upeta  : update eta upstream
    !
    !
    ...
    REAL(KIND=r8)   , INTENT(OUT  ) :: ueta  (ibdim,kmax)
    REAL(KIND=r8)   , INTENT(OUT  ) :: uetam (ibdim,kmax)
    REAL(KIND=r8)   , INTENT(IN   ) :: delt
    !
    !  local variables 
    !
    ...
    DO k=1,kmax
       DO i=1,ibdim
          uetam(i,k) = MIN(eta(kmax),MAX(eta(1),eta(k) - delt * w(i,k)))
       ENDDO
    ENDDO
    IF (final) THEN
       DO k=1,kmax
          DO i=1,ibdim
             ueta(i,k) = MIN(eta(kmax),MAX(eta(1),uetam(i,k)+uetam(i,k)-eta(k)))
          ENDDO
       ENDDO
    ENDIF
    !
  END SUBROUTINE Upeta
  ...
  SUBROUTINE Locate(&
    ...
    REAL(KIND=r8)   , INTENT(IN   ) :: ueta  (ibdim,kmax)
    REAL(KIND=r8)   , INTENT(INOUT) :: ulondp(ibdim,kmax,4)
    REAL(KIND=r8)   , INTENT(INOUT) :: ulatdp(ibdim,kmax)
    REAL(KIND=r8)   , INTENT(INOUT) :: uetadp(ibdim,kmax)
    !
    ...
    IF (leta) THEN
       DO k=1,kmax
          DO i=1,ibdim
             ik = (ueta(i,k)-etac(i,1)) / deletaref
             kloc(i,k) = ketaref(i,ik)
             IF (ueta(i,k).gt.etac(i,kloc(i,k)+1)) kloc(i,k)=kloc(i,k)+1
             uetadp(i,k) = (ueta(i,k)-etac(i,kloc(i,k)))/deletac(i,kloc(i,k))
          ENDDO
       ENDDO
    ENDIF
  ...
  SUBROUTINE InterpluvG(&
    ...
    REAL(KIND=r8)   , INTENT(IN   ) :: uetadp(ibdim,kmax)
    ...
    DO k=1,kmax
       DO i=1,ibdim
          ...
          f1 = s1 * (f1 + uetadp(i,k) * (f3-f1))
          f2 = s2 * (f2 + uetadp(i,k) * (f4-f2))
          g1 = s1 * (g1 + uetadp(i,k) * (g3-g1))
          g2 = s2 * (g2 + uetadp(i,k) * (g4-g2))
  ...
  SUBROUTINE InterplG(&
    ...
    DO k=1,kmax
       DO i=1,ibdim
          ...
          f1 = f1 + uetadp(i,k) * (f3-f1)
          f2 = f2 + uetadp(i,k) * (f4-f2)
  ...
  SUBROUTINE Interpcublin3d(&
    ...
    DO km=1,kmax
       ...
       DO i=1,ibdim
          ...
          ELSE
             fint(i,km) = fint2(i) + uetadp(i,km) * ( fint3(i)-fint2(i) )
          ENDIF
       ENDDO
       IF (lim) THEN
          DO i=1,ibdim
  ...
  SUBROUTINE InitSL
    ...
    ALLOCATE(etac(ibmax,1:kmax))
    ALLOCATE(deletac(ibmax,1:kmax))
    ...
    IF (SL_twotime_scheme) THEN
       ...
       ALLOCATE(ueta   (ibmax*kmax,jbmax))
      ...
      ELSE
       ALLOCATE(uetam  (ibmax*kmax,jbmax))
    END IF
    ...
    deletaref = del_eta(1)
    DO k=2,kmax-1
       deletaref = MIN(deletaref,del_eta(k))
    ENDDO
    delphiref = delphiref/2.0_r8
    deletaref = deletaref/2.0_r8
    kmaxref = ( eta(kmax)-eta(1) ) / deletaref + 1
    jmaxref = pai / delphiref + 1
    delphiref = pai / jmaxref
    deletaref = ( eta(kmax)-eta(1) ) / kmaxref
 
    ALLOCATE(jphiref(ibmax,0:jmaxref))
    ALLOCATE(ketaref(ibmax,0:kmaxref))
    ...
    j = 1
    DO k=1,kmaxref
       IF (k*deletaref+eta(1).gt.eta(j+1)) j = j + 1
       j = MIN(j,kmax-1)
       ketaref(1,k) = j
    ENDDO
    ...
    ENDDO
    DO j=2,kmax-2
       p1 = del_eta(j+1) / del_eta(j)
       p2 = del_eta(j-1) / del_eta(j)
       cubcoefv(1,j,1) = p1
       cubcoefv(1,j,2) = p2
    ...
    DO k=1,kmax-1
       DO i=1,ibmax
          etac(i,k) = eta(k)
          deletac(i,k) = del_eta(k)
       ENDDO
    ENDDO
    DO i=1,ibmax
       etac(i,kmax) = eta(kmax)
    ENDDO
