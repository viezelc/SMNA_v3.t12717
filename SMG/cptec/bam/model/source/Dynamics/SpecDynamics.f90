...
MODULE SpecDynamics
  ...
  USE Options,    ONLY : &
       ...
       MasCon,           & ! intent(in)
       MasCon_ps,        & ! intent(in)
       grid_difus,       & ! intent(in)
       eps_sic,          & ! intent(in)
       rhdifd,           & ! intent(in)
       rhdift,           & ! intent(in)
       Gamma_Factor,     & ! intent(in) 
       q_order             ! intent(in)
...
CONTAINS
  ...
  SUBROUTINE InitImplDifu(ct_in, cq_in, dk_in, tk_in,dt)
    ...
    BarDX = sqrt((4.0*pai*(er**2))/(iMax*jMax))

    ! tau=7200 damping time sacle of 2.5 hours dx=240 km dt 1800seg factor Gamma_Factor=6.25e-3 and q=2 
    tau = (dt/Gamma_Factor)/(((2.0)**(2*q_order+1)))

    gamma =dt/(((2.0)**(2*q_order+1)) * tau )

    k2qh= gamma * ((BarDX)**(2*q_order))/dt

    tk_grid= k2qh/(er**2)**ndho
    ...
    !   do k=myfirstlev,mylastlev,1
    !       PRINT*,'k=',k,'ct=',ct_in(k),'cq=',cq_in(k),'dk=',dk,'tk=',tk
    !   end do
  ...
  SUBROUTINE ImplDifu(dt, mnRIFirst, mnRILast)
    ...
    INTEGER, INTENT(IN) :: mnRILast
    INTEGER :: mn, k
    REAL(KIND=r8) :: work(mnRIFirst:mnRILast),work_grid(mnRIFirst:mnRILast)
    ...
    !
    DO mn = mnRIFirst, mnRILast
          work_grid(mn) = dt*tk_grid*workImplDifu(mn)
    END DO
    DO mn = mnRIFirst, mnRILast
    ...
    IF (grid_difus) THEN
       DO k = 1, kMaxloc
          DO mn = mnRIFirst, mnRILast
             qtmpp(mn,k) = qtmpp(mn,k)/(1.0_r8+work_grid(mn))
          END DO
       END DO
       DO mn = mnRIFirst, mnRILast
          qlnp_nabla4(mn) = qlnpp(mn)*work_grid(mn)
       END DO
    ELSE
       DO k = 1, kMaxloc
          DO mn = mnRIFirst, mnRILast
  ...
  SUBROUTINE bmcm(dt)
    ...
    REAL(KIND=r8) :: temp, det, dt2eps
    REAL(KIND=r8)    :: rim(kMax,kMax)
    !
    bm = 0.0
    dt2eps = eps_sic * dt * dt
    DO i=1, kmax
       IF (i.gt.1) THEN
          temp = rk * tref * rpi_r(i) / delp_r(i)
          DO j=1,i-1 
             bm(i,j) = temp * delp_r(j)
          END DO
       ENDIF
       bm(i,i) = alpha_r(i) * rk * tref
    END DO
...
!TO!cdir novector
    ...
    DO k=1, kmax
       DO mn = mnfirst_si,mnlast_si
          ...
          aux(mn,k)=aux(mn,k)*dteps*snnp1_si(mn)
       END DO
    END DO
    DO k=1, kmax
       DO mn = mnfirst_si,mnlast_si
...
!TO!cdir novector
    ...
    DO k=1, kmax
       DO mn = mnfirst_si,mnlast_si
          qtmpt_si(mn,k)=qtmpt_si(mn,k)-dteps*aux(mn,k)
       END DO
    END DO
    DO mn = mnfirst_si,mnlast_si
       aux(mn,1)=0.0_r8
...
!TO!cdir novector
    ...
    END DO
    DO mn = mnfirst_si,mnlast_si
       qlnpt_si(mn)=qlnpt_si(mn)-dteps*aux(mn,1)
    END DO
    ...
    ! Implementa Conservacao de ln(ps)
       ...
       IF (slagr) THEN
          DO mn = startMasCon,mnlast
             qlnpl(mn)=qlnpt(mn)
             qlnpp(mn)=qlnpl(mn)-qgzs(mn)/(gasr*tref)
          END DO
       ELSE
          DO mn = startMasCon,mnlast
             qlnpp(mn)=qlnpt(mn)
...
!TO!cdir novector
    ...
    DO k=1, kmax
       DO mn = mnRIFirst, mnRILast
          ...
          aux(mn,k)=aux(mn,k)*dteps*snnp1(mn)
       END DO
    END DO
    DO k=1, kmax
       DO mn = mnRIFirst, mnRILast
...
!TO!cdir novector
    ...
    DO k=1, kmax
       DO mn = mnRIFirst, mnRILast
          qtmpp(mn,k)=qtmpt(mn,k)-dteps*aux(mn,k)
       END DO
    END DO
    DO mn = mnRIFirst, mnRILast
       aux(mn,1)=0.0_r8
...
!TO!cdir novector
    ...
    IF (slagr) THEN
       DO mn = startMasCon, mnRILast
          qlnpl(mn)=qlnpt(mn)-dteps*aux(mn,1)
          qlnpp(mn)=qlnpl(mn)-qgzs(mn)/(gasr*tref)
       END DO
    ELSE
       DO mn = startMasCon, mnRILast
          qlnpp(mn)=qlnpt(mn)-dteps*aux(mn,1)
       END DO
    ENDIF
