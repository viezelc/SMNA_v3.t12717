...
! --------------------------------------------------------------------
      subroutine reftra_sw(               & !       reftra_sw ( &
                           nlayers      , & !              nlayers               , &
                           lrtchk       , & !              lrtchkclr(1:nlayers)  , &
                           pgg          , & !              zgcc     (1:nlayers)  , &
                           prmuz        , & !              prmu0                 , &
                           ptau         , & !              ztauc    (1:nlayers)  , &
                           pw           , & !              zomcc    (1:nlayers)  , &
                           pref         , & !              zrefc    (1:nlayers+1), &
                           prefd        , & !              zrefdc   (1:nlayers+1), &
                           ptra         , & !              ztrac    (1:nlayers+1), &
                           ptrad          ) !              ztradc   (1:nlayers+1)  )
! --------------------------------------------------------------------
...
! ------- Input -------
      ...
      logical, intent(in) :: lrtchk(1:nlayers)                           ! Logical flag for reflectivity and
                                                                 ! and transmissivity calculation; 
                                                                 !   Dimensions: (nlayers)

      real(kind=r8), intent(in) :: pgg(1:nlayers)                      ! asymmetry parameter
                                                                 !   Dimensions: (nlayers)
      real(kind=r8), intent(in) :: ptau(1:nlayers)                 ! optical depth
                                                                 !   Dimensions: (nlayers)
      real(kind=r8), intent(in) :: pw(1:nlayers)                     ! single scattering albedo 
                                                                 !   Dimensions: (nlayers)
      real(kind=r8), intent(in) :: prmuz                       ! cosine of solar zenith angle

! ------- Output -------

      real(kind=r8), intent(inout) :: pref(1:nlayers+1)                    ! direct beam reflectivity
                                                                 !   Dimensions: (nlayers+1)
      real(kind=r8), intent(inout) :: prefd(1:nlayers+1)                ! diffuse beam reflectivity
                                                                 !   Dimensions: (nlayers+1)
      real(kind=r8), intent(inout) :: ptra(1:nlayers+1)                    ! direct beam transmissivity
                                                                 !   Dimensions: (nlayers+1)
      real(kind=r8), intent(inout) :: ptrad(1:nlayers+1)                  ! diffuse beam transmissivity
                                                                 !   Dimensions: (nlayers+1)
