...
! --------------------------------------------------------------------------
      subroutine vrtqdr_sw(          &
                           ngptsw  , &!            ngptsw                              , &
                           klev    , &!            klev                              , &
                           kw      , &!            iw                                   , &
                           pref    , &!            zrefc      (1:klev+1)             , &
                           prefd   , &!            zrefdc     (1:klev+1)             , &
                           ptra    , &!            ztrac      (1:klev+1)             , &
                           ptrad   , &!            ztradc     (1:klev+1)             , &
                           pdbt    , &!            zdbtc      (1:klev+1)             , &
                           prdnd   , &!            zrdndc     (1:klev+1)             , &
                           prup    , &!            zrupc      (1:klev+1)             , &
                           prupd   , &!            zrupdc     (1:klev+1)             , &
                           ptdbt   , &!            ztdbtc     (1:klev+1)             , &
                           pfd     , &!            zcd        (1:klev+1,1:ngptsw)    , &
                           pfu       )!            zcu        (1:klev+1,1:ngptsw)      )
! --------------------------------------------------------------------------
...
! Input
      ...
      real(kind=r8), intent(in) :: pref  (1:klev+1)                     ! direct beam reflectivity
                                                                 !   Dimensions: (nlayers+1)
      real(kind=r8), intent(in) :: prefd (1:klev+1)                    ! diffuse beam reflectivity
                                                                 !   Dimensions: (nlayers+1)
      real(kind=r8), intent(in) :: ptra  (1:klev+1)                     ! direct beam transmissivity
                                                                 !   Dimensions: (nlayers+1)
      real(kind=r8), intent(in) :: ptrad (1:klev+1)                    ! diffuse beam transmissivity
                                                                 !   Dimensions: (nlayers+1)

      real(kind=r8), intent(in) :: pdbt  (1:klev+1) 
                                                                 !   Dimensions: (nlayers+1)
      real(kind=r8), intent(in) :: ptdbt (1:klev+1) 
                                                                 !   Dimensions: (nlayers+1)

      real(kind=r8), intent(inout) :: prdnd (1:klev+1) 
                                                                 !   Dimensions: (nlayers+1)
      real(kind=r8), intent(inout) :: prup  (1:klev+1) 
                                                                 !   Dimensions: (nlayers+1)
      real(kind=r8), intent(inout) :: prupd (1:klev+1) 
                                                                 !   Dimensions: (nlayers+1)

! Output
      real(kind=r8), intent(out) :: pfd(1:klev+1,1:ngptsw)                   ! downwelling flux (W/m2)
                                                                 !   Dimensions: (nlayers+1,ngptsw)
                                                                 ! unadjusted for earth/sun distance or zenith angle
      real(kind=r8), intent(out) :: pfu(1:klev+1,1:ngptsw)                   ! upwelling flux (W/m2)
                                                                 !   Dimensions: (nlayers+1,ngptsw)
                                                                 ! unadjusted for earth/sun distance or zenith angle
