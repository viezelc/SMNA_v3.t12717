...
!----------------------------------------------------------------------------
      subroutine taumol_sw(           &!      taumol_sw( &                                    !      taumol_sw( &
                           nlayers  , &!             nlayers                       , &        !              klev        , &
                           ngptsw   , &!             ngptsw                        , &        !              ngptsw                          , &
                           colh2o   , &!             colh2o   (1:nlayers)          , &        !              colh2o   (1:nlayers)            , &
                           colco2   , &!             colco2   (1:nlayers)          , &        !              colco2   (1:nlayers)            , &
                           colch4   , &!             colch4   (1:nlayers)          , &        !              colch4   (1:nlayers)            , &
                           colo2    , &!             colo2    (1:nlayers)          , &        !              colo2    (1:nlayers)            , &
                           colo3    , &!             colo3    (1:nlayers)          , &        !              colo3    (1:nlayers)            , &
                           colmol   , &!             colmol   (1:nlayers)          , &        !              colmol   (1:nlayers)            , &
                           laytrop  , &!             laytrop                       , &        !              laytrop                         , &
                           jp       , &!             jp       (1:nlayers)          , &        !              jp       (1:nlayers)            , &
                           jt       , &!             jt       (1:nlayers)          , &        !              jt       (1:nlayers)            , &
                           jt1      , &!             jt1      (1:nlayers)          , &        !              jt1      (1:nlayers)            , &
                           fac00    , &!             fac00    (1:nlayers)          , &        !              fac00    (1:nlayers)            , &
                           fac01    , &!             fac01    (1:nlayers)          , &        !              fac01    (1:nlayers)            , &
                           fac10    , &!             fac10    (1:nlayers)          , &        !              fac10    (1:nlayers)            , &
                           fac11    , &!             fac11    (1:nlayers)          , &        !              fac11    (1:nlayers)            , &
                           selffac  , &!             selffac  (1:nlayers)          , &        !              selffac  (1:nlayers)            , &
                           selffrac , &!             selffrac (1:nlayers)          , &        !              selffrac (1:nlayers)            , &
                           indself  , &!             indself  (1:nlayers)          , &        !              indself  (1:nlayers)            , &
                           forfac   , &!             forfac   (1:nlayers)          , &        !              forfac   (1:nlayers)            , &
                           forfrac  , &!             forfrac  (1:nlayers)          , &        !              forfrac  (1:nlayers)            , &
                           indfor   , &!             indfor   (1:nlayers)          , &        !              indfor   (1:nlayers)            , &
                           sfluxzen , &!             sfluxzen (1:ngptsw)           , &        !              zsflxzen (1:ngptsw)             , &
                           taug     , &!             taug     (1:nlayers,1:ngptsw) , &        !              ztaug    (1:nlayers,1:ngptsw)   , &
                           taur       )!             taur     (1:nlayers,1:ngptsw)   )        !              ztaur    (1:nlayers,1:ngptsw)     )
!----------------------------------------------------------------------------
...
! ----- Output -----
      real(kind=r8), intent(out) :: sfluxzen(1:ngptsw)          ! solar source function
                                                           !   Dimensions: (ngptsw)
      real(kind=r8), intent(out) :: taug(1:nlayers,1:ngptsw)             ! gaseous optical depth 
                                                           !   Dimensions: (nlayers,ngptsw)
      real(kind=r8), intent(out) :: taur(1:nlayers,1:ngptsw)             ! Rayleigh 
                                                           !   Dimensions: (nlayers,ngptsw)
...
! Calculate gaseous optical depth and planck fractions for each spectral band.
      ...
      call taumol16(laytrop,nlayers,ngptsw,sfluxzen,taug,taur)
      call taumol17(laytrop,nlayers,ngptsw,sfluxzen,taug,taur)
      call taumol18(laytrop,nlayers,ngptsw,sfluxzen,taug,taur)
      call taumol19(laytrop,nlayers,ngptsw,sfluxzen,taug,taur)
      call taumol20(laytrop,nlayers,ngptsw,sfluxzen,taug,taur)
      call taumol21(laytrop,nlayers,ngptsw,sfluxzen,taug,taur)
      call taumol22(laytrop,nlayers,ngptsw,sfluxzen,taug,taur)
      call taumol23(laytrop,nlayers,ngptsw,sfluxzen,taug,taur)
      call taumol24(laytrop,nlayers,ngptsw,sfluxzen,taug,taur)
      call taumol25(laytrop,nlayers,ngptsw,sfluxzen,taug,taur)
      call taumol26(laytrop,nlayers,ngptsw,sfluxzen,taug,taur)
      call taumol27(laytrop,nlayers,ngptsw,sfluxzen,taug,taur)
      call taumol28(laytrop,nlayers,ngptsw,sfluxzen,taug,taur)
      call taumol29(laytrop,nlayers,ngptsw,sfluxzen,taug,taur)
...
!----------------------------------------------------------------------------
      subroutine taumol16(laytrop,nlayers,ngptsw,sfluxzen,taug,taur)
!----------------------------------------------------------------------------
!
...
! ------- Declarations -------
      ...
      INTEGER      , INTENT(IN   ) :: ngptsw
      real(kind=r8), intent(out  ) :: sfluxzen (1:ngptsw)          ! solar source function
      real(kind=r8), intent(out  ) :: taug     (1:nlayers,1:ngptsw)             ! gaseous optical depth  Dimensions: (nlayers,ngptsw)
      real(kind=r8), intent(out  ) :: taur     (1:nlayers,1:ngptsw)             ! Rayleigh ! Dimensions: (nlayers,ngptsw)
...
!----------------------------------------------------------------------------
      subroutine taumol17(laytrop,nlayers,ngptsw,sfluxzen,taug,taur)
!----------------------------------------------------------------------------
!
...
!----------------------------------------------------------------------------
      subroutine taumol18(laytrop,nlayers,ngptsw,sfluxzen,taug,taur)
!----------------------------------------------------------------------------
!
...
!----------------------------------------------------------------------------
      subroutine taumol19(laytrop,nlayers,ngptsw,sfluxzen,taug,taur)
!----------------------------------------------------------------------------
!
...
!----------------------------------------------------------------------------
      subroutine taumol20(laytrop,nlayers,ngptsw,sfluxzen,taug,taur)
!----------------------------------------------------------------------------
!
...
!----------------------------------------------------------------------------
      subroutine taumol21(laytrop,nlayers,ngptsw,sfluxzen,taug,taur)
!----------------------------------------------------------------------------
!
...
!----------------------------------------------------------------------------
      subroutine taumol22(laytrop,nlayers,ngptsw,sfluxzen,taug,taur)
!----------------------------------------------------------------------------
!
...
!----------------------------------------------------------------------------
      subroutine taumol23(laytrop,nlayers,ngptsw,sfluxzen,taug,taur)
!----------------------------------------------------------------------------
!
...
!----------------------------------------------------------------------------
      subroutine taumol24(laytrop,nlayers,ngptsw,sfluxzen,taug,taur)
!----------------------------------------------------------------------------
!
...
!----------------------------------------------------------------------------
      subroutine taumol25(laytrop,nlayers,ngptsw,sfluxzen,taug,taur)
!----------------------------------------------------------------------------
!
...
!----------------------------------------------------------------------------
      subroutine taumol26(laytrop,nlayers,ngptsw,sfluxzen,taug,taur)
!----------------------------------------------------------------------------
!
...
!----------------------------------------------------------------------------
      subroutine taumol27(laytrop,nlayers,ngptsw,sfluxzen,taug,taur)
!----------------------------------------------------------------------------
!
...
!----------------------------------------------------------------------------
      subroutine taumol28(laytrop,nlayers,ngptsw,sfluxzen,taug,taur)
!----------------------------------------------------------------------------
!
...
!----------------------------------------------------------------------------
      subroutine taumol29(laytrop,nlayers,ngptsw,sfluxzen,taug,taur)
!----------------------------------------------------------------------------
!
