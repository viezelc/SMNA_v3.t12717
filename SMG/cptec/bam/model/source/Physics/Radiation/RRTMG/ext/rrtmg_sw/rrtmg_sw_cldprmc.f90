...
! ----------------------------------------------------------------------------
      subroutine cldprmc_sw( &        !              cldprmc_sw( &
                            nlayers, &!                      nlayers                                    , &
                            inflag , &!                      inflag                                     , &
                            iceflag, &!                      iceflag                                    , &
                            liqflag, &!                      liqflag                                    , &
                            cldfmc , &!                      cldfmc              (1:ngptsw,1:nlayers)   , &
                            ciwpmc , &!                      ciwpmc              (1:ngptsw,1:nlayers)   , &
                            clwpmc , &!                      clwpmc              (1:ngptsw,1:nlayers)   , &
                            reicmc , &!                      reicmc              (1:nlayers)            , &
                            dgesmc , &!                      dgesmc              (1:nlayers)            , &
                            relqmc , &!                      relqmc              (1:nlayers)            , &
                            taormc , &!                      taormc              (1:ngptsw,1:nlayers)   , & 
                            taucmc , &!                      taucmc              (1:ngptsw,1:nlayers)   , & 
                            ssacmc , &!                      ssacmc              (1:ngptsw,1:nlayers)   , & 
                            asmcmc , &!                      asmcmc              (1:ngptsw,1:nlayers)   , & 
                            fsfcmc   )!                      fsfcmc              (1:ngptsw,1:nlayers)   ) 
! ----------------------------------------------------------------------------
...
! ------- Input -------
      ...
      real(kind=r8), intent(in) :: cldfmc(1:ngptsw,1:nlayers)        ! cloud fraction [mcica]
                                                        !    Dimensions: (ngptsw,nlayers)
      real(kind=r8), intent(in) :: ciwpmc(1:ngptsw,1:nlayers)        ! cloud ice water path [mcica]
                                                        !    Dimensions: (ngptsw,nlayers)
      real(kind=r8), intent(in) :: clwpmc(1:ngptsw,1:nlayers)        ! cloud liquid water path [mcica]
                                                        !    Dimensions: (ngptsw,nlayers)
      real(kind=r8), intent(in) :: relqmc(1:nlayers)           ! cloud liquid particle effective radius (microns)
                                                        !    Dimensions: (nlayers)
      real(kind=r8), intent(in) :: reicmc(1:nlayers)           ! cloud ice particle effective radius (microns)
                                                        !    Dimensions: (nlayers)
      real(kind=r8), intent(in) :: dgesmc(1:nlayers)           ! cloud ice particle generalized effective size (microns)
                                                        !    Dimensions: (nlayers)
      real(kind=r8), intent(in) :: fsfcmc(1:ngptsw,1:nlayers)        ! cloud forward scattering fraction 
                                                        !    Dimensions: (ngptsw,nlayers)

! ------- Output -------

      real(kind=r8), intent(inout) :: taucmc(1:ngptsw,1:nlayers)     ! cloud optical depth (delta scaled)
                                                        !    Dimensions: (ngptsw,nlayers)
      real(kind=r8), intent(inout) :: ssacmc(1:ngptsw,1:nlayers)     ! single scattering albedo (delta scaled)
                                                        !    Dimensions: (ngptsw,nlayers)
      real(kind=r8), intent(inout) :: asmcmc(1:ngptsw,1:nlayers)     ! asymmetry parameter (delta scaled)
                                                        !    Dimensions: (ngptsw,nlayers)
      real(kind=r8), intent(out) :: taormc(1:ngptsw,1:nlayers)       ! cloud optical depth (non-delta scaled)
                                                        !    Dimensions: (ngptsw,nlayers)
