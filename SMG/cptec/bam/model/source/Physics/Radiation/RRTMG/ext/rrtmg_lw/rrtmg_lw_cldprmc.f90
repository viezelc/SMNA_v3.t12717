...
! ------------------------------------------------------------------------------
      subroutine cldprmc( &        !       call cldprmc( &
                         nlayers, &!                 nlayers                      , &
                         inflag , &!                 inflag                       , &
                         iceflag, &!                 iceflag                      , &
                         liqflag, &!                 liqflag                      , &
                         cldfmc , &!                 cldfmc   (1:ngptlw,1:nlayers), &
                         ciwpmc , &!                 ciwpmc   (1:ngptlw,1:nlayers), &
                         clwpmc , &!                 clwpmc   (1:ngptlw,1:nlayers), &
                         reicmc , &!                 reicmc   (1:nlayers)         , &
                         dgesmc , &!                 dgesmc   (1:nlayers)         , &
                         relqmc , &!                 relqmc   (1:nlayers)         , &
                         ncbands, &!                 ncbands                      , &
                         taucmc   )!                 taucmc   (1:ngptlw,1:nlayers)  )
! ------------------------------------------------------------------------------
...
! ------- Input -------
      ...
      real(kind=r8), intent(in) :: cldfmc(1:ngptlw,1:nlayers)          ! cloud fraction [mcica]
                                                        !    Dimensions: (ngptlw,nlayers)
      real(kind=r8), intent(in) :: ciwpmc(1:ngptlw,1:nlayers)          ! cloud ice water path [mcica]
                                                        !    Dimensions: (ngptlw,nlayers)
      real(kind=r8), intent(in) :: clwpmc(1:ngptlw,1:nlayers)          ! cloud liquid water path [mcica]
                                                        !    Dimensions: (ngptlw,nlayers)
      real(kind=r8), intent(in) :: relqmc(1:nlayers)           ! liquid particle effective radius (microns)
                                                        !    Dimensions: (nlayers)
      real(kind=r8), intent(in) :: reicmc(1:nlayers)            ! ice particle effective radius (microns)
                                                        !    Dimensions: (nlayers)
      real(kind=r8), intent(in) :: dgesmc(1:nlayers)           ! ice particle generalized effective size (microns)
                                                        !    Dimensions: (nlayers)

! ------- Output -------

      integer, intent(out) :: ncbands                   ! number of cloud spectral bands
      real(kind=r8), intent(inout) :: taucmc (1:ngptlw,1:nlayers)        ! cloud optical depth [mcica]
                                                        !    Dimensions: (ngptlw,nlayers)
