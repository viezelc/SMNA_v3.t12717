...
! Public subroutines
!------------------------------------------------------------------
!          call mcica_subcol_lw( ncol                                                 , &
!                                rrtmg_levs-1                                         , &
!                                icld                                                 , &
!                                permuteseed                                          , &
!                                pmid        (1:pcols,1:rrtmg_levs-1)                 , &
!                                cld         (1:pcols,1:rrtmg_levs-1)                 , &
...
!                                rei         (1:pcols,1:rrtmg_levs-1)                 , &   ! ice particle effective radius (microns)
!                                rel         (1:pcols,1:rrtmg_levs-1)                 , &   ! liquid particle radius (micron)
!                                tauc_lw     (1:nbndlw ,1: pcols,1: rrtmg_levs-1)     , &   ! Cloud longwave optical depth by band
...
!                                rei_stolw   (1:pcols,1:rrtmg_levs-1)                 , &
!                                rel_stolw   (1:pcols,1:rrtmg_levs-1)                 , &
!                                tauc_stolw  (1:nsubclw,1: pcols,1: rrtmg_levs-1)       )

      subroutine mcica_subcol_lw(ncol                                                , & !integer, intent(in) :: ncol            ! number of columns
                                 ...
                                 play                                                , & !real(kind=r8), intent(in) :: play(ncol,nlay)          ! layer pressures (mb) 
                                 cldfrac                                             , & !real(kind=r8), intent(in) :: cldfrac(ncol,nlay)       ! layer cloud fraction
                                 ciwp                                                , & !real(kind=r8), intent(in) :: ciwp (ncol,nlay)          ! cloud ice water path
                                 clwp                                                , & !real(kind=r8), intent(in) :: clwp (ncol,nlay)          ! cloud liquid water path
                                 rei                                                 , & !real(kind=r8), intent(in) :: rei (ncol,nlay)           ! cloud ice particle size
                                 rel                                                 , & !real(kind=r8), intent(in) :: rel (ncol,nlay)          ! cloud liquid particle size
                                 tauc                                                , & !real(kind=r8), intent(in) :: tauc (nbndlw,ncol,nlay)        ! cloud optical depth
                                 cldfmcl                                             , & !real(kind=r8), intent(out) :: cldfmcl(ngptlw,ncol,nlay)    ! cloud fraction [mcica]
                                 ciwpmcl                                             , & !real(kind=r8), intent(out) :: ciwpmcl(ngptlw,ncol,nlay)    ! cloud ice water path [mcica]
                                 clwpmcl                                             , & !real(kind=r8), intent(out) :: clwpmcl(ngptlw,ncol,nlay)    ! cloud liquid water path [mcica]
                                 reicmcl                                             , & !real(kind=r8), intent(out) :: reicmcl (ncol,nlay)      ! ice partcle size (microns)
                                 relqmcl                                             , & !real(kind=r8), intent(out) :: relqmcl (ncol,nlay)      ! liquid particle size (microns)
                                 taucmcl                                               ) !real(kind=r8), intent(out) :: taucmcl(ngptlw,ncol,nlay)    ! cloud optical depth [mcica]
...
! Atmosphere
      real(kind=r8), intent(in) :: play(ncol,nlay)          ! layer pressures (mb) 
                                                        !    Dimensions: (ncol,nlay)

! Atmosphere/clouds - cldprop
      real(kind=r8), intent(in) :: cldfrac(ncol,nlay)       ! layer cloud fraction
                                                        !    Dimensions: (ncol,nlay)
      real(kind=r8), intent(in) :: tauc (nbndlw,ncol,nlay)        ! cloud optical depth
                                                        !    Dimensions: (nbndlw,ncol,nlay)
!      real(kind=r8), intent(in) :: ssac(nbndlw,ncol,nlay)       ! cloud single scattering albedo
                                                        !    Dimensions: (nbndlw,ncol,nlay)
!      real(kind=r8), intent(in) :: asmc(nbndlw,ncol,nlay)       ! cloud asymmetry parameter
                                                        !    Dimensions: (nbndlw,ncol,nlay)
      real(kind=r8), intent(in) :: ciwp (ncol,nlay)          ! cloud ice water path
                                                        !    Dimensions: (ncol,nlay)
      real(kind=r8), intent(in) :: clwp (ncol,nlay)          ! cloud liquid water path
                                                        !    Dimensions: (ncol,nlay)
      real(kind=r8), intent(in) :: rei (ncol,nlay)           ! cloud ice particle size
                                                        !    Dimensions: (ncol,nlay)
      real(kind=r8), intent(in) :: rel (ncol,nlay)          ! cloud liquid particle size
                                                        !    Dimensions: (ncol,nlay)

! ----- Output -----
! Atmosphere/clouds - cldprmc [mcica]
      real(kind=r8), intent(out) :: cldfmcl(ngptlw,ncol,nlay)    ! cloud fraction [mcica]
                                                        !    Dimensions: (ngptlw,ncol,nlay)
      real(kind=r8), intent(out) :: ciwpmcl(ngptlw,ncol,nlay)    ! cloud ice water path [mcica]
                                                        !    Dimensions: (ngptlw,ncol,nlay)
      real(kind=r8), intent(out) :: clwpmcl(ngptlw,ncol,nlay)    ! cloud liquid water path [mcica]
                                                        !    Dimensions: (ngptlw,ncol,nlay)
      real(kind=r8), intent(out) :: relqmcl (ncol,nlay)      ! liquid particle size (microns)
                                                        !    Dimensions: (ncol,nlay)
      real(kind=r8), intent(out) :: reicmcl (ncol,nlay)      ! ice partcle size (microns)
                                                        !    Dimensions: (ncol,nlay)
      real(kind=r8), intent(out) :: taucmcl(ngptlw,ncol,nlay)    ! cloud optical depth [mcica]
                                                        !    Dimensions: (ngptlw,ncol,nlay)
!      real(kind=r8), intent(out) :: ssacmcl((ngptlw,ncol,nlay)   ! cloud single scattering albedo [mcica]
                                                        !    Dimensions: (ngptlw,ncol,nlay)
!      real(kind=r8), intent(out) :: asmcmcl(ngptlw,ncol,nlay)   ! cloud asymmetry parameter [mcica]
                                                        !    Dimensions: (ngptlw,ncol,nlay)
...
! Stochastic cloud generator variables [mcica]
      ...
      integer :: km, im, nm ,k,i                 ! loop indices
...
!  Generate the stochastic subcolumns of cloud optical properties for the longwave;
      call generate_stochastic_clouds (     &
                               ncol       , & !integer, intent(in) :: ncol         ! number of columns
                               nlay       , & !integer, intent(in) :: nlay         ! number of layers
                               nsubclw    , & !integer, intent(in) :: nsubcol         ! number of sub-columns (g-point intervals)
                               icld       , & !integer, intent(in) :: icld         ! clear/cloud, cloud overlap flag
                               pmid       , & !real(kind=r8), intent(in) :: pmid(ncol,nlay)        ! layer pressure (Pa)
                               cldfrac    , & !real(kind=r8), intent(in) :: cld(ncol,nlay)        ! cloud fraction 
                               clwp       , & !real(kind=r8), intent(in) :: clwp(ncol,nlay)        ! cloud liquid water path
                               ciwp       , & !real(kind=r8), intent(in) :: ciwp(ncol,nlay)        ! cloud ice water path
                               tauc       , & !real(kind=r8), intent(in) :: tauc(nbndlw,ncol,nlay)     ! cloud optical depth
                               cldfmcl    , & !real(kind=r8), intent(out) :: cld_stoch (ngptlw,ncol,nlay)  ! subcolumn cloud fraction 
                               clwpmcl    , & !real(kind=r8), intent(out) :: clwp_stoch(ngptlw,ncol,nlay) ! subcolumn cloud liquid water path
                               ciwpmcl    , & !real(kind=r8), intent(out) :: ciwp_stoch(ngptlw,ncol,nlay) ! subcolumn cloud ice water path
                               taucmcl    , & !real(kind=r8), intent(out) :: tauc_stoch(ngptlw,ncol,nlay) ! subcolumn cloud optical depth
                               permuteseed  ) !integer, optional, intent(in) :: changeSeed     ! allows permuting seed
...
!-------------------------------------------------------------------------------------------------
      subroutine generate_stochastic_clouds( &
                               ncol       , &!integer, intent(in) :: ncol            ! number of columns
                               nlay       , &!integer, intent(in) :: nlay            ! number of layers
                               nsubcol    , &!integer, intent(in) :: nsubcol         ! number of sub-columns (g-point intervals)
                               icld       , &!integer, intent(in) :: icld            ! clear/cloud, cloud overlap flag
                               pmid       , &!real(kind=r8), intent(in) :: pmid(ncol,nlay)          ! layer pressure (Pa)
                               cld        , &!real(kind=r8), intent(in) :: cld(ncol,nlay)           ! cloud fraction 
                               clwp       , &!real(kind=r8), intent(in) :: clwp(ncol,nlay)          ! cloud liquid water path
                               ciwp       , &!real(kind=r8), intent(in) :: ciwp(ncol,nlay)          ! cloud ice water path
                               tauc       , &!real(kind=r8), intent(in) :: tauc(nbndlw,ncol,nlay)        ! cloud optical depth
                               cld_stoch  , &!real(kind=r8), intent(out) :: cld_stoch (ngptlw,ncol,nlay)  ! subcolumn cloud fraction 
                               clwp_stoch , &!real(kind=r8), intent(out) :: clwp_stoch(ngptlw,ncol,nlay) ! subcolumn cloud liquid water path
                               ciwp_stoch , &!real(kind=r8), intent(out) :: ciwp_stoch(ngptlw,ncol,nlay) ! subcolumn cloud ice water path
                               tauc_stoch , &!real(kind=r8), intent(out) :: tauc_stoch(ngptlw,ncol,nlay) ! subcolumn cloud optical depth
                               changeSeed   )!integer, optional, intent(in) :: changeSeed     ! allows permuting seed
!-------------------------------------------------------------------------------------------------
...
! Column state (cloud fraction, cloud water, cloud ice) + variables needed to read physics state 
      real(kind=r8), intent(in) :: pmid(ncol,nlay)          ! layer pressure (Pa)
                                                        !    Dimensions: (ncol,nlay)
      real(kind=r8), intent(in) :: cld(ncol,nlay)           ! cloud fraction 
                                                        !    Dimensions: (ncol,nlay)
      real(kind=r8), intent(in) :: clwp(ncol,nlay)          ! cloud liquid water path
                                                        !    Dimensions: (ncol,nlay)
      real(kind=r8), intent(in) :: ciwp(ncol,nlay)          ! cloud ice water path
...
!      real(kind=r8), intent(in) :: asmc(nbndlw,ncol,nlay)       ! cloud asymmetry parameter
      ...
      real(kind=r8), intent(out) :: cld_stoch (ngptlw,ncol,nlay)  ! subcolumn cloud fraction 
                                                        !    Dimensions: (ngptlw,ncol,nlay)
      real(kind=r8), intent(out) :: clwp_stoch(ngptlw,ncol,nlay) ! subcolumn cloud liquid water path
                                                        !    Dimensions: (ngptlw,ncol,nlay)
      real(kind=r8), intent(out) :: ciwp_stoch(ngptlw,ncol,nlay) ! subcolumn cloud ice water path
                                                        !    Dimensions: (ngptlw,ncol,nlay)
      real(kind=r8), intent(out) :: tauc_stoch(ngptlw,ncol,nlay) ! subcolumn cloud optical depth
                                                        !    Dimensions: (ngptlw,ncol,nlay)
!      real(kind=r8), intent(out) :: ssac_stoch(ngptlw,ncol,nlay)! subcolumn cloud single scattering albedo
                                                        !    Dimensions: (ngptlw,ncol,nlay)
                                                        !   inactive - for future expansion
!      real(kind=r8), intent(out) :: asmc_stoch(ngptlw,ncol,nlay)! subcolumn cloud asymmetry parameter
                                                        !    Dimensions: (ngptlw,ncol,nlay)
                                                        !   inactive - for future expansion
