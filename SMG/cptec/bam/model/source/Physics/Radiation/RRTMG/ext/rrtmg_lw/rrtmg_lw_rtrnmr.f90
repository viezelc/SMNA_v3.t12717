...
!-----------------------------------------------------------------------------
      subroutine rtrnmr( &           ! call rtrnmr( &
                        ...
                        taut      , &!         taut       (1:nlayers,1:ngptlw)   , &
                        totuflux  , &!         totuflux   (0:nlayers)            , &
                        totdflux  , &!         totdflux   (0:nlayers)            , &
                        fnet      , &!         fnet       (0:nlayers)            , &
                        htr       , &!         htr        (0:nlayers)            , &
                        totuclfl  , &!         totuclfl   (0:nlayers)            , &
                        totdclfl  , &!         totdclfl   (0:nlayers)            , &
                        fnetc     , &!         fnetc      (0:nlayers)            , &
                        htrc      , &!         htrc       (0:nlayers)            , &
                        totufluxs , &!         totufluxs  (1:nbndlw,0:nlayers)   , &
                        totdfluxs   )!         totdfluxs  (1:nbndlw,0:nlayers)     )
!-----------------------------------------------------------------------------
!
...
! Atmosphere
      ...
      real(kind=r8), intent(in) :: semiss(1:nbndlw)            ! lw surface emissivity
                                                        !    Dimensions: (nbndlw)
      ...
      real(kind=r8), intent(in) :: plankbnd(1:nbndlw)           ! 
                                                        !    Dimensions: (nbndlw)
      real(kind=r8), intent(in) :: fracs(1:nlayers,1:ngptlw)           ! 
                                                        !    Dimensions: (nlayers,ngptw)
      real(kind=r8), intent(in) :: taut(1:nlayers,1:ngptlw)            ! gaseous + aerosol optical depths
                                                        !    Dimensions: (nlayers,ngptlw)

! Clouds
      integer, intent(in) :: ncbands                    ! number of cloud spectral bands
      real(kind=r8), intent(in) :: cldfrac(1:nlayers)            ! layer cloud fraction
                                                        !    Dimensions: (nlayers)
      real(kind=r8), intent(in) :: taucloud(1:nlayers ,1:nbndlw)         ! layer cloud optical depth
                                                        !    Dimensions: (nlayers,nbndlw)
! ----- Output -----
      real(kind=r8), intent(out) :: totuflux(0:nlayers)        ! upward longwave flux (w/m2)
                                                        !    Dimensions: (0:nlayers)
      real(kind=r8), intent(out) :: totdflux(0:nlayers)        ! downward longwave flux (w/m2)
                                                        !    Dimensions: (0:nlayers)
      real(kind=r8), intent(out) :: fnet(0:nlayers)            ! net longwave flux (w/m2)
                                                        !    Dimensions: (0:nlayers)
      real(kind=r8), intent(out) :: htr(0:nlayers)             ! longwave heating rate (k/day)
                                                        !    Dimensions: (0:nlayers)
      real(kind=r8), intent(out) :: totuclfl(0:nlayers)        ! clear sky upward longwave flux (w/m2)
                                                        !    Dimensions: (0:nlayers)
      real(kind=r8), intent(out) :: totdclfl(0:nlayers)        ! clear sky downward longwave flux (w/m2)
                                                        !    Dimensions: (0:nlayers)
      real(kind=r8), intent(out) :: fnetc(0:nlayers)           ! clear sky net longwave flux (w/m2)
                                                        !    Dimensions: (0:nlayers)
      real(kind=r8), intent(out) :: htrc(0:nlayers)            ! clear sky longwave heating rate (k/day)
                                                        !    Dimensions: (0:nlayers)
      real(kind=r8), intent(out) :: totufluxs(1:nbndlw,0:nlayers)     ! upward longwave flux spectral (w/m2)
                                                        !    Dimensions: (nbndlw, 0:nlayers)
      real(kind=r8), intent(out) :: totdfluxs(1:nbndlw,0:nlayers)     ! downward longwave flux spectral (w/m2)
                                                        !    Dimensions: (nbndlw, 0:nlayers)
...
! Maximum/random cloud overlap
            ...
            faccmb1(lev+1) = facclr1(lev+1) * faccld2(lev) * cldfrac(lev) 
            faccmb2(lev+1) = faccld1(lev+1) * facclr2(lev) * (1._r8 - cldfrac(lev)) 
         else
            istcld(lev+1) = 1
         endif
      enddo
         ...
         if (icldlyr(lev).eq.1) then
            ...
            elseif (cldfrac(lev-1) .ge. cldfrac(lev)) then
               ...
               if (istcldd(lev) .eq. 1) then
                  ...
                  fmax = max(cldfrac(lev-1),cldfrac(lev))
                  if (cldfrac(lev-1) .gt. fmax) then
                     facclr1d(lev-1) = rat2
                     facclr2d(lev-1) = (cldfrac(lev-1)-fmax)/(1._r8-fmax)
                  elseif (cldfrac(lev-1) .lt. fmax) then
                     facclr1d(lev-1) = (cldfrac(lev-2)-cldfrac(lev-1))/ &
               ...
               if (istcldd(lev) .eq. 1) then
                  ...
                  fmin = min(cldfrac(lev-1),cldfrac(lev))
                  if (cldfrac(lev-1) .le. fmin) then
                     faccld1d(lev-1) = rat1
            ...
            faccmb1d(lev-1) = facclr1d(lev-1) * faccld2d(lev) * cldfrac(lev) 
            faccmb2d(lev-1) = faccld1d(lev-1) * facclr2d(lev) * (1._r8 - cldfrac(lev))
         else
            istcldd(lev-1) = 1
         endif
      enddo
