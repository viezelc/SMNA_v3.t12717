...
CONTAINS
  ...
  SUBROUTINE Init_Optical_Properties(dtpg,NLAY,a_hybr,b_hybr,nbndsw_in,nbndlw_in,fNameCldOptSW , &
             fNameCldOptLW )
    IMPLICIT NONE
    REAL(KIND=r8)   , INTENT(in) :: dtpg
    INTEGER         , INTENT(in) :: NLAY          !   NLAY            : vertical layer number                             !
    REAL(KIND=r8)   , INTENT(in) :: a_hybr(NLAY+1)
    REAL(KIND=r8)   , INTENT(in) :: b_hybr(NLAY+1)
    ...
    nlwbands = nbndlw_in
    !...................................
    CALL cldinit(dtpg,NLAY,a_hybr,b_hybr)
    !-----------------------------------
    !
...
!    REAL(KIND=r8) :: cwp    (IM,LM)! in-cloud cloud (total) water path
  ...
  SUBROUTINE Run_Optical_Properties(iswrad,ncol,pcols,pver,cldfsnow_idx,cldfsnow,cldn,ql,qi,rel,rei,&
       cliqwp,iciwp,clwp,pdel,lamc,pgam,c_cld_tau ,c_cld_tau_w , c_cld_tau_w_g,c_cld_tau_w_f, &
       c_cld_lw_abs,cldfprime ,landfrac, dosw,dolw,&
    ...
    REAL(KIND=r8), INTENT(IN    ) :: landfrac (1:pcols)   ! Land fraction
    ...
    IF (dosw) THEN
       IF(oldcldoptics) THEN
          CALL ec_ice_optics_sw(iswrad,ncol,pcols,pver,cldn,rei,iciwp,qi,pdel,&
               ice_tau, ice_tau_w, ice_tau_w_g, ice_tau_w_f,landfrac, oldicewp=.FALSE.)
          CALL slingo_liq_optics_sw(iswrad,ncol,pcols,pver,cldn,rel,iclwp,cliqwp,ql,pdel,&
               liq_tau, liq_tau_w, liq_tau_w_g, liq_tau_w_f, landfrac,oldliqwp=.FALSE.)
    ...
    IF (dolw) THEN
       ...
       ELSE
          ...
          SELECT CASE (TRIM(liqcldoptics))
          CASE ('slingo')
             CALL slingo_liq_get_rad_props_lw(iswrad,landfrac,ncol,pcols,pver,iciwp,qi,ql,pdel,iclwp,iciwp, &
  ...
  SUBROUTINE slingo_liq_get_rad_props_lw(iswrad,landfrac,ncol,pcols,pver,cldn,qi,ql,pdel,iclwpth,iciwpth,&
       abs_od, oldliqwp)
    ...
    IF (oldliqwp) THEN
       DO k=1,pver
          DO i = 1,ncol
             gicewp(i,k) = qi(i,k)*pdel(i,k)/SHR_CONST_G*1000.0_r8  ! Grid box ice water path. g/m**2
             gliqwp(i,k) = ql(i,k)*pdel(i,k)/SHR_CONST_G*1000.0_r8  ! Grid box liquid water path.  g/m**2
             cicewp(i,k) = gicewp(i,k) /  MAX(0.1_r8,(cldn(i,k)))                  ! In-cloud ice water path.
             cliqwp(i,k) = gliqwp(i,k) /  MAX(0.1_r8,(cldn(i,k)))             ! In-cloud liquid water path. 
 ...
 !            ficemr(i,k) = 1000.0_r8 * iciwpth(i,k)/(MAX(1.e-18_r8, cwp(i,k)))
             IF (TRIM(iswrad).eq.'RRTMG') THEN
                ...
                !   cicewp(i,k) = iciwpth(i,k)! / MAX(0.1_r8,(1.0_8-cldn(i,k)))                 ! In-cloud ice water path.
                !   cliqwp(i,k) = iclwpth(i,k)! / MAX(0.1_r8,(1.0_8-cldn(i,k)))                 ! In-cloud liquid water path.
                !   ficemr(i,k) = qi(i,k) /MAX(1.e-10_r8,(qi(i,k)+ql(i,k)))
                !   cwp   (i,k) = cliqwp(i,k) +  cicewp(i, k)
!
 !                  !ficemr(i,k) = cicewp(i,k)/(MAX(1.e-18_r8, cwp(i,k)))
 !                ELSE
             ...
             ELSE
                IF(landfrac(i)>0.5_r8)THEN
                 ...
                 ELSE
                   gicewp(i,k) = qi(i,k)*pdel(i,k)/SHR_CONST_G*1000.0_r8  ! Grid box ice water path. g/m**2
                   gliqwp(i,k) = ql(i,k)*pdel(i,k)/SHR_CONST_G*1000.0_r8  ! Grid box liquid water path.  g/m**2
                   cicewp(i,k) = gicewp(i,k) /  MAX(0.1_r8,(1.0_8-cldn(i,k)))                  ! In-cloud ice water path.
                   cliqwp(i,k) = gliqwp(i,k) /  MAX(0.1_r8,(1.0_8-cldn(i,k)))             ! In-cloud liquid water path. 
                   ficemr(i,k) = qi(i,k) /MAX(1.e-10_r8,(qi(i,k)+ql(i,k)))
    ...
    ELSE
       ...
       DO k=1,pver
          DO i = 1,ncol
             ...
             cicewp(i,k) = iciwpth(i,k) / MAX(0.1_r8,(cldn(i,k)))! In-cloud ice water path.
             cliqwp(i,k) = iclwpth(i,k) / MAX(0.1_r8,(cldn(i,k)))! In-cloud liquid water path.
             cwp(i,k)    = cicewp(i,k) + cliqwp(i,k)
             ficemr(i,k) = cicewp(i,k)/(MAX(1.e-18_r8,cwp(i,k)))
...
!    INTEGER :: k           ! lev index
  ...
  SUBROUTINE cloud_rad_props_get_sw(iswrad,ncol,pcols,pver,cldn,rei,iciwp,qi,pdel, &
       tau, tau_w, tau_w_g,landfrac, tau_w_f)
...
!    INTEGER :: iswband ! sw band indices
    ...
    CALL ec_ice_optics_sw   (iswrad,ncol,pcols,pver,cldn,rei,iciwp,qi,pdel, tau, tau_w,&
         tau_w_g, tau_w_f,landfrac, oldicewp=.FALSE.)
  ...
  SUBROUTINE slingo_cloud_rad_props_get_sw(iswrad,ncol,pcols,pver,cldn,rel,iclwp,ql,pdel,&
       tau, tau_w, tau_w_g,landfrac, tau_w_f)
...
!    REAL(r8) :: liq_tau_w_f(nswbands,pcols,pver) ! aerosol forward scattered fraction * tau * w
    ...
    CALL slingo_liq_optics_sw( iswrad,ncol,pcols,pver,cldn,rel,iclwp,cliqwp,ql,pdel,&
         tau, tau_w, tau_w_g, tau_w_f,landfrac, oldliqwp=.FALSE. )
  ...
  SUBROUTINE ec_ice_optics_sw   (iswrad,ncol,pcols,pver,cldn,rei,iciwp,qi,pdel,ice_tau, ice_tau_w,&
       ice_tau_w_g, ice_tau_w_f,landfrac, oldicewp)
    ...
    IF(oldicewp) THEN
       DO k=1,pver
          DO i = 1,Nday
             cicewp(i,k) =  ((1000.0_r8*qi(i,k)*pdel(i,k) /(SHR_CONST_G* MAX(0.01_r8,cldn(i,k)))))! g/m2
          END DO
       END DO
    ELSE
       !     if (iciwp_idx<=0) then 
...
!    end do
       ...
       DO k=1,pver
          DO i = 1,Nday
             !cicewp(i,k) =  1000.0_r8*iciwp(i,k)
             IF (TRIM(iswrad).eq.'RRTMG') THEN
                !IF(landfrac(i)>0.5_r8)THEN
                    cicewp(i,k) =  (1000.0_r8*qi(i,k)*pdel(i,k) /(SHR_CONST_G*(MAX(0.01_r8,(1.0_8-cldn(i,k))))))! In-cloud ice water path.[ g/m2 ]
                !ELSE
                !    cicewp(i,k) =  iciwp(i,k) /((1.0_r8 - MAX(0.01_r8,cldn(i,k)))) ! In-cloud ice water path.[ g/m2 ]
                !END IF
             ELSE
                !IF(landfrac(i)>0.5_r8)THEN
                   cicewp(i,k) =  ((1000.0_r8*qi(i,k)*pdel(i,k) /(SHR_CONST_G*(MAX(0.1_r8,(1.0_8-cldn(i,k)))))))  ![ g/m2 ]
!                  cicewp(i,k) =          ((1000.0_r8*qi(i,k)*pdel(i,k) /(SHR_CONST_G*(1.0_r8 - MAX(0.01_r8,cldn(i,k))))))! In-cloud ice water path.[ g/m2 ]
                !ELSE
                !   cicewp(i,k) =  iciwp (i,k)/1000.0_r8 ! g/m2  ->  kg/m2
                !   cicewp(i,k) =  cicewp(i,k)/(MAX(0.1_r8,(1.0_8-cldn(i,k))))! In-cloud ice water path.[ g/m2 ]
                !END IF
             END IF
             !print*,landfrac(i),cicewp(i,k),qi(i,k),cldn(i,k)
          END DO
       END DO
    ENDIF
    ...
    DO ns = 1, nswbands
       ...
       DO k=1,pver
          DO i=1,Nday
             ! claculate optical depth or optical thickness, 
             ! note that optical properties for ice valid only
             ! in range of 13 > rei > 130 micron (Ebert and Curry 92)
  ...
  SUBROUTINE slingo_liq_optics_sw(iswrad,ncol,pcols,pver,cldn,rel,iclwp,cliqwp2,ql,pdel,&
       liq_tau, liq_tau_w, liq_tau_w_g, liq_tau_w_f, landfrac,oldliqwp)
    ...
    REAL(r8),INTENT(IN   ) :: cldn(pcols,pver)
    REAL(r8),INTENT(IN   ) :: rel (pcols,pver)
    REAL(r8),INTENT(IN   ) :: iclwp(pcols,pver)     !g/m2
    REAL(r8),INTENT(IN   ) :: cliqwp2(pcols,pver)   !g/m2
    REAL(r8),INTENT(IN   ) :: ql   (pcols,pver)
    REAL(r8),INTENT(IN   ) :: pdel  (pcols,pver)             ! Pa
...
!
!          ! gicewp = (QCF(i,k)+qs1d(k)) * pdel(ncol,k)*100.0 / gravmks * 1000.0     ! Grid box ice water path.
!          gicewp = (QCF(i,k)) * pdel(i,k) / con_g * 1000.0_r8     ! Grid box ice water path.  g/m2 
!          gliqwp = (QCL(i,k)) * pdel(i,k) / con_g * 1000.0_r8       ! Grid box liquid water path. g/m2 
!          cicewp(i,k) = gicewp / max(0.01_r8,cldfrac(i,k))                    ! In-cloud ice water path.  g/m2 
!          cliqwp(i,k) = gliqwp / max(0.01_r8,cldfrac(i,k))                    ! In-cloud liquid water path. g/m2 
!       END DO
!    end do
!             IF (cldn(i,k) >= cldmin .AND. cldn(i,k) >= cldeps) THEN
       ...
       DO k=1,pver
          DO i = 1,Nday
             IF (TRIM(iswrad).eq.'RRTMG') THEN
                IF(landfrac(i)>0.5_r8)THEN
                   cliqwp(i,k) = ql(i,k)*pdel(i,k)/(SHR_CONST_G*(MAX(0.01_r8,(1.0_8-cldn(i,k))))) !  kg/m2
                ELSE
                   cliqwp(i,k) = (cliqwp2(i,k)/1000.0)              ! g/m2  ->  kg/m2
                   cliqwp(i,k) =  cliqwp(i,k)/MAX(0.1_r8,(1.0_8-cldn(i,k)))! kg/m2   In-cloud ice water path.
                END IF
                !PRINT*,landfrac(i),cliqwp(i,k)
              ELSE
                !IF(landfrac(i)>0.5_r8)THEN
                !   cliqwp(i,k) = ql(i,k)*pdel(i,k)/(SHR_CONST_G*(MAX(0.01_r8,(1.0_8-cldn(i,k))))) !  kg/m2
                !ELSE
                   cliqwp(i,k) = (cliqwp2(i,k)/1000.0)              ! g/m2   ->  kg/m2
                   cliqwp(i,k) =  cliqwp(i,k)/MAX(0.1_r8,(1.0_8-cldn(i,k)))
                !END IF
             END IF
          END DO
       END DO
...
!    REAL(r8), DIMENSION(pcols,pver) :: kext
  ...
  !...................................
  SUBROUTINE cldinit(dtpg,NLAY,a_hybr,b_hybr)
    !-----------------------------------
    REAL(KIND=r8)   , INTENT(in) ::  dtpg
    INTEGER, INTENT(in) :: NLAY !   NLAY            : vertical layer number                             !
    REAL(KIND=r8) , INTENT(in) :: a_hybr(NLAY+1)
...
!!  SB  Changed to use with hybrid coordinates . These are ordered
!!  SB  from top to bottom, and inverted to use with iflip = 1
!!
    DO k=1,NLAY+1
      IF (iflip == 0) THEN
         si(k) = a_hybr(k)/1.e5_r8+b_hybr(k)
        ELSE
         kl = nlay+2-k
         si(k) = a_hybr(kl)/1.e5_r8+b_hybr(kl)
      ENDIF
      !if(myid.eq.0) write(0,*) 'si(k) top-down rad_optics',k,si(k)
    ENDDO
