...
CONTAINS
  ...
  !==========================================================================
    ...
    REAL(KIND=r8):: taucldorig2  (1:kMax,1:jpband) 
    REAL(KIND=r8):: taucloud    (nCols,1:kMax,1:jpband)
    REAL(KIND=r8):: taucloud2   (nCols,1:kMax,1:jpband)
    ...
    ELSE IF(icld.eq.4 .or. icld == 5.or. icld == 6 .or. icld == 7) THEN
       ...
       DO i=1,nsol
          call cldprop_sw( &                                     !             cldprop_sw(
                            0                                 , &! integer, intent(in)  :: inflag    =2!0    ! see definitions
                            0                                 , &! integer, intent(in)  :: iceflag   =0!0    ! see definitions
                            0                                 , &! integer, intent(in)  :: liqflag   =1!0    ! see definitions
                            kmax                              , &!          kmax   , &
                            cldfrac  (          i,1:kmax)     , &! real(kind=r8), intent(in) :: cldfrac   (1:kmax)                 , &
                            tauc_sw   (1:nbndsw,i,1:kmax)     , &! real(kind=r8), intent(in) :: tauc_sw   (1:nbndsw,iplon,1:kmax)  , &
                            ssac_sw   (1:nbndsw,i,1:kmax)     , &! real(kind=r8), intent(in) :: ssac_sw   (1:nbndsw,iplon,1:kmax)  , &
                            asmc_sw   (1:nbndsw,i,1:kmax)     , &! real(kind=r8), intent(in) :: asmc_sw   (1:nbndsw,iplon,1:kmax)  , &
                            cicewp    (         i,1:kmax)     , &! real(kind=r8), intent(in) :: cicewp    (iplon,1:kmax)               , &
                            cliqwp    (         i,1:kmax)     , &! real(kind=r8), intent(in) :: cliqwp    (iplon,1:kmax)               , &
                            reicmcl   (         i,1:kmax)     , &! real(kind=r8), intent(in) :: reicmcl   (iplon,1:kmax)               , &
                            2*reicmcl (         i,1:kmax)     , &! real(kind=r8), intent(in) :: 2*reicmcl (iplon,1:kmax)               , &
                            relqmcl   (         i,1:kmax)     , &! real(kind=r8), intent(in) :: relqmcl   (iplon,1:kmax)               , &
                            taucldorig(1:kmax,1:jpband)       , &!          taucldorig(1:kmax,1:jpband)          , &
                            taucloud  (i,1:kmax,1:jpband)       )!          taucloud  (1:kmax,1:jpband)          , &

          call cldprop_sw( &                                     !             cldprop_sw(
                            2                                 , &! integer, intent(in)  :: inflag    =2!0    ! see definitions
                            0                                 , &! integer, intent(in)  :: iceflag   =0!0    ! see definitions
                            1                                 , &! integer, intent(in)  :: liqflag   =1!0    ! see definitions
                            kmax                              , &!          kmax   , &
                            cldfrac   (         i,1:kmax)     , &! real(kind=r8), intent(in) :: cldfrac   (1:kmax)                 , &
                            tauc_sw   (1:nbndsw,i,1:kmax)     , &! real(kind=r8), intent(in) :: tauc_sw   (1:nbndsw,iplon,1:kmax)  , &
                            ssac_sw   (1:nbndsw,i,1:kmax)     , &! real(kind=r8), intent(in) :: ssac_sw   (1:nbndsw,iplon,1:kmax)  , &
                            ...
                            2*reicmcl (         i,1:kmax)     , &! real(kind=r8), intent(in) :: 2*reicmcl (iplon,1:kmax)               , &
                            relqmcl   (         i,1:kmax)     , &! real(kind=r8), intent(in) :: relqmcl   (iplon,1:kmax)               , &
                            taucldorig2(1:kmax,1:jpband)      , &!          taucldorig(1:kmax,1:jpband)          , &
                            taucloud2 (i,1:kmax,1:jpband)       )!          taucloud  (1:kmax,1:jpband)          , & 
        END DO

        DO i=1,nsol
          DO k=1,kmax
             DO ns = 1, jpband
                 !TauCloudTot(i,k)=TauCloudTot(i,k)+((0.5_r8*taucloud (i,k,ns)+0.5_r8*taucloud2 (i,k,ns)))
                 TauCloudTot(i,k)=TauCloudTot(i,k)+(taucloud (i,k,ns))

                 ! TauCloudTot(i,k)=TauCloudTot(i,k)+MAX(taucloud (i,k,ns),taucloud2 (i,k,ns))
             END DO
          ENDDO
       END DO
...
! ----------------------------------------------------------------------------
      subroutine cldprop_sw( &             !             cldprop_sw(
                            inflag      , &! integer, intent(in)  :: inflag    =2!0    ! see definitions
                            iceflag     , &! integer, intent(in)  :: iceflag   =0!0    ! see definitions
                            liqflag     , &! integer, intent(in)  :: liqflag   =1!0    ! see definitions
                            nlayers     , &! integer, intent(in) :: nlayers   , &
                            cldfrac     , &! real(kind=r8), intent(in) :: cldfrac   (1:nlayers)                 , &
                            tauc        , &! real(kind=r8), intent(in) :: tauc_sw   (1:nbndsw,iplon,1:nlayers)  , &
                            ssac        , &! real(kind=r8), intent(in) :: ssac_sw   (1:nbndsw,iplon,1:nlayers)  , &
                            asmc        , &! real(kind=r8), intent(in) :: asmc_sw   (1:nbndsw,iplon,1:nlayers)  , &
                            ciwp        , &! real(kind=r8), intent(in) :: cicewp    (iplon,1:nlayers)               , &
                            clwp        , &! real(kind=r8), intent(in) :: cliqwp    (iplon,1:nlayers)               , &
                            rei         , &! real(kind=r8), intent(in) :: reicmcl   (iplon,1:nlayers)               , &
                            dge         , &! real(kind=r8), intent(in) :: 2*reicmcl (iplon,1:nlayers)               , &
                            rel         , &! real(kind=r8), intent(in) :: relqmcl   (iplon,1:nlayers)               , &
                            taucldorig  , &! real(kind=r8), intent(out) :: taucldorig(1:nlayers,1:jpband)          , &
                            taucloud      )! real(kind=r8), intent(out) :: taucloud  (1:nlayers,1:jpband)          , &
! ----------------------------------------------------------------------------
...
! ------- Input -------
      ...
      integer, intent(in)  :: inflag    !0    ! see definitions
      integer, intent(in)  :: iceflag   !0    ! see definitions
      integer, intent(in)  :: liqflag   !0    ! see definitions
