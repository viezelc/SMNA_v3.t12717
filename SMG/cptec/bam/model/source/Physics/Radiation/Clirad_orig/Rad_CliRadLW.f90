...
!PK              ELSE
!PK !---
!PK                IF (cld(i,k).GT.clu(i,k).AND.cld(i,k).GT.0.01_r8) THEN 
!PK                    fcld(i,k+1)=cld(i,k)
!PK 
!PK                    IF (ta(i,k+1).GT.253.0_r8) THEN
!PK                       taucl(i,k+1,2)=0.05_r8*TauCloudTot(i,k)
!PK                    ELSE
!PK                       taucl(i,k+1,1)=0.025_r8*TauCloudTot(i,k)
!PK                    ENDIF
!PK 
!PK                 ENDIF
!PK                 IF (clu(i,k).GE.cld(i,k).AND.clu(i,k).GT.0.01_r8) THEN 
!PK                    fcld(i,k+1)=clu(i,k)
!PK                    taucl(i,k+1,2)=0.05_r8*TauCloudTot(i,k)
!PK                 ENDIF
!PK 
!PK              END IF 
    ...
    ELSE
       DO k=1,np
          DO i=1,m
             IF(imask  (i) > 0_i8 .and. pl20 (i,k) < 75.0_r8)THEN
             ...
             ELSE
                ...
                IF (cld(i,k).GT.clu(i,k).AND.cld(i,k).GT.0.01_r8) THEN 
                   fcld(i,k+1)=cld(i,k)
                ...
                ENDIF
                IF (clu(i,k).GE.cld(i,k).AND.clu(i,k).GT.0.01_r8) THEN 
                   fcld(i,k+1)=clu(i,k)
                   taucl(i,k+1,2)=0.05_r8*dp(i,k+1)*1.16_r8
                ENDIF
  ...
  SUBROUTINE h2oexps(ib      , &! INTEGER, INTENT(IN   ) :: ib!  spectral band (ib)
    ...
    REAL(KIND=r8), PARAMETER :: xkw_CoefABS(8) =RESHAPE(SOURCE=(/&
  ...
  SUBROUTINE conexps( ib     ,&!INTEGER, INTENT(IN   ) :: ib!  spectral band (ib)
    ...
    REAL(KIND=r8), PARAMETER :: xke_CoefABS(8) =RESHAPE(SOURCE=(/&
  ...
  SUBROUTINE tablup( &
    ...
    DO i=1,m
       ...
       x1=MAX(sabs(i,k2)-sabs(i,k1),0.001_r8)
       x2=MAX(spre(i,k2)-spre(i,k1),0.001_r8)/x1
