...
!    REAL(KIND=r8)   ,INTENT(INOUT) :: RAINNC     (1:nCols)
    ...
    REAL(KIND=r8)   ,INTENT(in   ) :: prsi    (1:nCols,kMax+1)  
    REAL(KIND=r8)   ,INTENT(in   ) :: prsl    (1:nCols,kMax  ) 
    ...
    REAL(KIND=r8)  :: qv_phy    (1:nCols,1:kMax)
    REAL(KIND=r8)  :: qc_phy    (1:nCols,1:kMax)
    REAL(KIND=r8)  :: qi_phy    (1:nCols,1:kMax)
    REAL(KIND=r8)  :: qr_phy    (1:nCols,1:kMax)
    ...
    CALL GSMDRIVE( &
      ...
      qv_phy     , &!real (kind=r8) :: qv    (nCols,kMax)
      qc_phy     , &!(1:nCols,1:kMax)
      qi_phy     , &!(1:nCols,1:kMax)
      qr_phy     , &!(1:nCols,1:kMax)
...
!
    ...
    DO k = 1,kMax
       DO i = 1,nCols
          ...
          dtdt (i,k)=(t_phy  (i,k) -gt(i,k))/DT
          dqdt (i,k)=(qv_phy (i,k) -qv(i,k))/DT
          dqldt(i,k)=(qc_phy (i,k) -qc(i,k))/DT
          dqidt(i,k)=(qi_phy (i,k) -qi(i,k))/DT
          dqrdt(i,k)=(qr_phy (i,k) -qr(i,k))/DT
...
!      real (kind=r8) :: tc, wc, qi, qr, qw
      ...
      real (kind=r8) :: rhpbl 
      real (kind=r8) :: rhbtop
      real (kind=r8) :: work1(nCols),  work2(nCols),coslat(nCols)
      ...
      REAL(KIND=r8)   :: DeltaP (nCols,1:kMax)    !GFDL
      REAL(KIND=r8)   :: sik    (nCols,1:kMax+1)
      REAL(KIND=r8)   :: sikp1  (nCols,1:kMax+1)
      REAL(KIND=r8)   :: slk    (nCols,1:kMax) 
      REAL(KIND=r8)   :: RKAPI
      REAL(KIND=r8)   :: RKAPP1
!
!------------------------------------------------------------------------
...
!      enddo
      ...
      do i = 1, nCols
       ! colrad.....colatitude  colrad=0 - 3.14 (0-180)from np to sp in radians
        ...
        work1(i)    = abs(log(coslat(i) / (nlons*latg)) - dxmin) * dxinv
...
!
  ...
  SUBROUTINE GSMCONST (dt)
    ...
    !      ice is =0.1_r8 below:
    !PK CIACW=DTPH*0.25_r8*con_pi*0.1_r8*(1.E5_r8)**C1! original
         CIACW = 0.10_r8      ! Brad's suggestion 20040614
